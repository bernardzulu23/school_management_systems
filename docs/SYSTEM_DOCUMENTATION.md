...
| **Timetabling**             | Generate repairs legacy multi-class pushes via **`normalizePushedAllocations`** (upserts real `TeacherAllocation` per class — no synthetic IDs) → **`remapEntriesToValidAllocationIds`** before save → **`filterConflictFreeSchedulerEntries`** / **`isConflict`** / **`hasConflict`** (teacher+class+same day: same subject blocked unless lesson frequency exceeds working days; different subjects blocked only on period overlap). **Preflight** via **`runPreflightFeasibility`** + **`GET /api/timetable/feasibility`** blocks over-allocated teachers/classes before generation. Tiered solver timeout **`computeMaxExecutionMs`** (Vercel cap). Publish requires zero errors **and** warnings (`TimetableDraftMeta`). Scheduler **`canPlace`** + **`tryReassignTeacherClassBlocks`** backtrack; **`backtrackReassignAssignments`** in auto-resolve (dev console logs MOVED/STUCK). **`Class.isActive`** synced via **`syncClassActiveFlags`** (`assignmentsOnly` after generate) and **`cleanupStaleConflicts`** (deactivates empty classes, filters stale class conflicts from draft meta); timetable APIs default to **`getActiveClasses`** (teaching/timetable/allocation usage only). Class tabs use **`filterClassesForWallGrid`** (assignments only, deduped labels, no enrolment shells); wall grids use **`filterClassesForWallGrid`** and **`alignAssignmentsToBellRows`**. Headteacher dashboard **`TimetableSummary`** previews the **class wall** (`AscClassWallGrid`), loading **published** then **draft**. Period labels use **`formatPeriodConfigLabel`** with class count for multi-class allocations. Headteacher **Edit** tab + `/dashboard/headteacher/timetable/edit` → **Class wall** default (`AscClassWallGrid`). Client grid teacher colours: **`lib/timetable/teacherColorPalette.js`** (`buildTeacherColorMap`); server DB colours via **`lib/timetable/teacherColors.js`**. Secondary grades: **Forms 1–6** and **Grades 10–12** only (no Grade 8/9). | `lib/timetable/timetableStore.ts` (`reloadFromServer`), `lib/timetable/preflightFeasibility.ts`, `lib/timetable/solverTimeout.ts`, `lib/timetable/slotScoring.ts`, `lib/timetable/constraintCheck.ts`, `lib/timetable/activeClasses.ts`, `lib/timetable/cleanupConflicts.ts`, `lib/timetable/getActiveClasses.js`, `scripts/sync-class-active-flags.js`, `lib/timetable/gridHelpers.js`, `lib/timetable/formatPeriodConfig.js`, `lib/timetable/solverBacktrackResolve.ts`, `lib/timetable/scheduler.ts`, `lib/timetable/greedySolver.ts`, `lib/timetable/teacherColorPalette.js`, `app/api/timetable/feasibility/route.ts`, `lib/timetable/resolveDepartmentClasses.js`, `components/timetable/TimetableClassPicker.tsx`, `components/timetable/AscClassWallGrid.tsx`, `components/dashboard/TimetableSummary.js` |
...
| **Lesson plans**            | Authoring; **approval:** secondary → HOD, primary → senior teacher / deputy (else headteacher). Teaching Studio coverage: **manual week marks** and/or **APPROVED** lesson plans. Record of Work auto-fills from approved plans. Curriculum Studio **weeks/term** is a free number (1–20). Quiz maker uses loose `QuizGenerationSchema` + `parseQuizObject` with text JSON fallback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `LessonPlan.weekNumber`/`schemeId`, `lib/teaching/syncTaughtProgressFromLessonPlan.js`, `lib/lesson-plans/reviewer.js`, `lib/curriculum/recordOfWorkExport.ts`, `lib/ai/schemas.js` (`QuizGenerationSchema`), `app/api/ai/quiz-maker`, `components/teaching/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **AI features**             | Lesson planner, quizzes, topic tests (RAG material picker), report comments, RAG; **input guardrails** (`lib/ai/guardrails.ts`) block prompt-injection / non-education payloads; **semantic response cache** (`AiCache` + `lib/ai/cache.ts`) hashes payloads (SHA-256) before calling Groq/Gemini; structured quiz/ECZ outputs via Zod + `generateAIObject`; built-in **CDC 2024 Chemistry syllabus** (`data/curriculum/chemistry-cdc-2024.json`, 82 subtopic chunks via `lib/curriculum/`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `lib/ai/`, `lib/ai/guardrails.ts`, `lib/ai/cache.ts`, `AiCache`, `GET /api/materials/rag-preview`, `/dashboard/teacher/topic-test`, [AI_GUIDE.md](./AI_GUIDE.md), [RAG.md](./RAG.md)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Guidance teacher**        | Career guidance portal, cases, safeguarding, re-entry, resources (Phase 1–2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `app/api/guidance/` (14 routes), `lib/guidance/`, `/dashboard/guidance/*`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Regenerate the API catalog after route changes: `npm run docs:api-routes` → [API_ROUTES.md](./API_ROUTES.md) (**379** route files as of 2026-06-29).
...
| `/api/guidance`     |     14 | Guidance teacher portal — assignments, cases, re-entry, resources, reports |
...
| Guidance teacher (assignment) | `/dashboard/guidance`    | Career clusters, cases, safeguarding, re-entry, resources (`GuidanceAssignment` on `User`; headteacher assigns at `/dashboard/headteacher/guidance-teachers`). Pathname-based portal — not a separate JWT role. Server layouts + `lib/guidance/routeAuth.js`. |
