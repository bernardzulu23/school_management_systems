...
| **Timetabling**             | HOD allocations → hybrid generate → server conflict audit → Conflict Centre → publish; **double/triple cards** span multiple bell slots via `calcRowSpan` / `rowSpanForAssignment` (`lib/timetable/gridHelpers.js`); **Master** (`MasterTimetableGrid`) and **entity** (`TimetableEntityView`) views use a single parent CSS grid with `gridRow: span N` on day cells; **aSc class wall** (`AscClassWallGrid`) uses HTML `<table>` + `colSpan` for horizontal multi-period blocks; generator **day-spread** hard rule blocks stacking multi-period blocks for same class+subject or teacher on one day (`wouldStackSameDay` in `scheduler.ts`); Phase 3: draft-meta KPI, sidebar badge, PATCH/sync-draft hard gates | `lib/timetable/conflictAudit.js`, `lib/timetable/draftHardConflictCheck.js`, `lib/timetable/mapEntriesToAssignments.js`, `GET /api/timetable/draft-meta`, `GET /api/timetable/conflicts`, `POST /api/timetable/conflicts/resolve`, [03 timetable conflict resolution.md](./03%20timetable%20conflict%20resolution.md) |
