...
| **Timetabling**             | Generate repairs legacy multi-class pushes via **`normalizePushedAllocations`** (upserts real `TeacherAllocation` per class — no synthetic IDs) → **`remapEntriesToValidAllocationIds`** before save → **`filterConflictFreeSchedulerEntries`** / **`isConflict`** (day+period hard constraints). Headteacher **class-by-class grid**: `/dashboard/headteacher/timetable/class-view` (`TimetableClassView`, `TimetableClassGrid`, `lib/timetable/colorScheme.ts`). | `lib/timetable/normalizePushedAllocations.js`, `lib/timetable/constraintCheck.ts`, `lib/timetable/colorScheme.ts`, `components/timetable/TimetableClassView.tsx`, `app/dashboard/headteacher/timetable/class-view/page.tsx` |
