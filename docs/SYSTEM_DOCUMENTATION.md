...
| **Timetabling**             | HOD allocations → **preflight feasibility** (`lib/timetable/preflightFeasibility.ts` counts lesson **blocks** per class/teacher, not summed `consecutivePeriods`) → hybrid generate → server conflict audit → Conflict Centre → publish; **canonical edit grid** is `MasterTimetableGrid` with `@dnd-kit/core` drag-and-drop, `useTimetableStore` (`moveAssignment`, `swapAssignments`, `resetAssignments`), and optimistic persistence via `lib/timetable/timetableMutations.ts`; **double/triple cards** span multiple bell slots via `calcRowSpan` / `rowSpanForAssignment` (`lib/timetable/gridHelpers.js`); **entity** (`TimetableEntityView`) and **aSc class wall** (`AscClassWallGrid`) remain alternate layouts; `DELETE /api/timetable/entries` supports `{ clearAll: true, term, academicYear }` to wipe draft entries; legacy `DragDropTimetable` and `MasterTimetableGenerator` table removed | `lib/timetable/dragDropHelpers.ts`, `lib/timetable/preflightFeasibility.ts`, `lib/timetable/conflictAudit.js`, `lib/timetable/draftHardConflictCheck.js`, `GET /api/timetable/draft-meta`, `PATCH/DELETE /api/timetable/entries`, [03 timetable conflict resolution.md](./03%20timetable%20conflict%20resolution.md) |
