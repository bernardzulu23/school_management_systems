...
| **Timetabling**             | HOD allocations → approve/reject/clear → **`normalizePushedAllocations`** (one pushed row per class; repairs legacy single-row multi-grade approvals) → preflight → hybrid generate with **interleaved block ordering** (`lessonOrdering.ts`), **day/class spread**, dynamic break positions, greedy fallback → conflict audit → publish; edit grid: `MasterTimetableGrid` + DnD | `lib/timetable/normalizePushedAllocations.js`, `lib/timetable/periodExpansion.ts`, `lib/timetable/lessonOrdering.ts`, `lib/timetable/scheduler.ts`, `lib/timetable/greedySolver.ts`, `lib/timetable/hybridGenerate.ts`, [03 timetable conflict resolution.md](./03%20timetable%20conflict%20resolution.md) |
