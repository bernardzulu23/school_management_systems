...
| **Timetabling**             | HOD **Class Allocation** (`sessionFetch` — required for anti-scraping headers) → submit → headteacher **approve/reject/edit/delete** (`PUT/DELETE /api/admin/allocations/[id]`, resyncs pushed rows on approved edits) → **`normalizePushedAllocations`** → preflight → hybrid generate | `components/timetable/HODAllocationPage.js`, `app/api/admin/allocations/`, `lib/timetable/departmentAllocationMutations.js`, `lib/timetable/normalizePushedAllocations.js`, `lib/timetable/hybridGenerate.ts` |
