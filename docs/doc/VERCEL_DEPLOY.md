...
| Build OOM (`SIGKILL`, OOM in build report)  | `vercel.json` sets `NODE_OPTIONS=--max-old-space-size=6144` (leave ~2GB headroom under the 8GB builder) and `GENERATE_SOURCEMAP=false`. On Vercel, `webpackBuildWorker` is **off** (child + 6GB heap OOMs on 8GB). Sentry webpack plugins / map upload only when `SENTRY_UPLOAD_SOURCEMAPS=1`. If it still OOMs on Pro/Enterprise, set **Enhanced** or **Elastic** build machines (16GB+) in project Build settings. Redeploy after push. |
