...
| Build OOM (`SIGKILL`, OOM in build report)  | `vercel.json` sets `NODE_OPTIONS=--max-old-space-size=6144` (leave ~2GB headroom under the 8GB builder) and `GENERATE_SOURCEMAP=false`. Webpack uses `webpackBuildWorker` + `webpackMemoryOptimizations`, `parallelism: 1`, and Sentry source-map upload only when `SENTRY_UPLOAD_SOURCEMAPS=1`. If it still OOMs on Pro/Enterprise, enable **Enhanced builds** (16GB) in the Vercel project. Redeploy after push. |
