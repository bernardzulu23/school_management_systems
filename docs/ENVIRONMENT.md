...
| `CRON_SECRET`                                         | Vercel cron route auth                | Required for `/api/cron/*` (ECZ reminder, SMS low balance, fee overdue). Send as `Authorization: Bearer $CRON_SECRET` or `x-cron-secret` header — see [DEVELOPER_GUIDE.md](./DEVELOPER_GUIDE.md) |
