...
| `LIPILA_WEBHOOK_SECRET`                               | Authenticate Lipila POST callbacks              | Required in production. Accept `Authorization: Bearer`, `x-webhook-secret`, or `?webhook_secret=` (auto-appended to callback URLs). GET callbacks only redirect — they do not mark paid.                                                                                                                         |
| `AFRICASTALKING_API_KEY`                              | Outbound cloud SMS                              | Both key and username required; used after Android gateway (see [SMS_GUIDE.md](./SMS_GUIDE.md)). Alias: `AFRICAS_TALKING_API_KEY`                                                                                                                                                                                |
| `AFRICASTALKING_USERNAME`                             | Africa's Talking account                        | Sandbox available for testing; also used for bulk broadcast. Alias: `AFRICAS_TALKING_USERNAME`                                                                                                                                                                                                                   |
| `AFRICASTALKING_SENDER_ID`                            | Optional AT sender / shortcode                  | School-context SMS sender when set                                                                                                                                                                                                                                                                               |
| `SMS_WEBHOOK_SECRET`                                  | Authenticate SMS inbound/DLR webhooks           | Required for `/api/sms/inbound` and `/api/sms/delivery`. Same header/query forms as Lipila webhook secret. Configure AT callback URLs with `?webhook_secret=...` if headers cannot be set.                                                                                                                       |
...
| `VERCEL_API_TOKEN`                                    | Read Vercel Web Analytics for platform overview | [Vercel](https://vercel.com/account/tokens) → create token with access to the team/project. Required for `/platform/overview` website traffic KPIs.                                                                                                                                                              |
| `VERCEL_PROJECT_ID`                                   | Vercel project id for Analytics queries         | Project Settings → General (required; no hardcoded fallback)                                                                                                                                                                                                                                                     |
| `VERCEL_TEAM_ID`                                      | Vercel team id for Analytics queries            | Team Settings → General (`team_…`; required)                                                                                                                                                                                                                                                                     |
...
| `OPENROUTER_API_KEY`                                  | Chat fallback + RAG embeddings                  | [openrouter.ai](https://openrouter.ai) — chain order Groq → Gemini → **OpenRouter** → OpenAI → HF. Also powers `/v1/embeddings` when used for RAG.                                                                                                                                                               |
| `OPENROUTER_MODEL`                                    | OpenRouter chat model                           | Default: `openai/gpt-4o-mini`                                                                                                                                                                                                                                                                                    |
| `OPENROUTER_SITE_URL`                                 | OpenRouter HTTP-Referer header                  | Default: `https://bluepeacktechnologies.com`                                                                                                                                                                                                                                                                     |
| `OPENROUTER_APP_NAME`                                 | OpenRouter X-Title header                       | Default: `Zambian School Management System`                                                                                                                                                                                                                                                                      |
| `OPENAI_API_KEY`                                      | Chat fallback + RAG embeddings (paid)           | Later fallback after OpenRouter; embeddings `text-embedding-3-small` @ 384d                                                                                                                                                                                                                                      |
...
- Give **Production** and **Preview** different `JWT_SECRET`, `JWT_REFRESH_SECRET`, and preferably separate Neon branches / DB roles.
- Never copy Production DB URLs into Preview.
- Secrets (AT, Lipila, QStash, Resend, `CRON_SECRET`) should be Production-only or use provider sandboxes for Preview.
- See [NEON_DB_ROLES.md](./NEON_DB_ROLES.md) for least-privilege `zsms_app` vs `zsms_migrate`.
- See [CLOUDFLARE_RATE_LIMITING.md](./CLOUDFLARE_RATE_LIMITING.md) for WAF rules + SaaS custom hostnames (CSP/HSTS via `proxy.js` apply to all hostnames hitting this deployment).

See also [docs/doc/VERCEL_DEPLOY.md](./doc/VERCEL_DEPLOY.md), [NEON_POST_DEPLOY_CHECKLIST.md](./NEON_POST_DEPLOY_CHECKLIST.md), [NEON_DB_ROLES.md](./NEON_DB_ROLES.md), and [docs/doc/LEGACY_MISC_ARCHIVE.md](./doc/LEGACY_MISC_ARCHIVE.md).
