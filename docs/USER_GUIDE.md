...
9. Click **Publish** when there are **no hard conflicts** — teachers and students then see the published version. Publishing again for the same term **replaces** the previous published timetable (it does not stack a second copy). Dragging a lesson onto a slot that already overlaps another lesson for that class or teacher is **blocked immediately** (database rule), not only listed later on the Conflicts/audit screens.
