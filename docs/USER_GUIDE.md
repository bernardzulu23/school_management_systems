...
- **Login without subdomain:** On the apex or `www` marketing domain (`www.bluepeacktechnologies.com`), `/login` shows **Zambian School Management System** branding (not a school name). Use `/login?subdomain=<school-slug>` to target a specific school, or open `https://<subdomain>.bluepeacktechnologies.com/login`.
...
- **Trial SMS allowance:** New trial schools receive **50 free SMS credits** when the portal is created. Track remaining and used credits on `/dashboard/sms`. When the balance hits 0, sending is blocked and **Subscribe to continue** links to `/pricing`.
...
| Activity log        | `/dashboard/headteacher/activity`            |
...
4. Use **User Management** (`/dashboard/users`) to view, edit, or deactivate accounts — and to **Assign Guidance teachers**, **Assign SIC**, or open **Notifications**. Open **Activity log** (`/dashboard/headteacher/activity`) for a read-only, school-wide trail of who created, updated, deleted, or published records (filter by date, user, module, and action). HODs see a department-scoped view when activity is tagged to their department.
...
4. Click **Generate Perfect Timetable** (uses **all** HOD allocations for the term — newly approved **and** previously published/scheduled — plus the bell schedule). After the first publish, approving another department and regenerating still includes earlier departments so prior classes do not disappear from the draft. For a **new empty term**, use **Copy from previous term** instead of rebuilding from scratch — it copies class/subject/teacher structure into a new draft, remaps clocks to the **current bell schedule** (Timetable Settings), remaps allocation IDs to the target term’s HOD allocations, then rescans Conflicts (so invalid times after a bell change are not silently kept).
5. Review the draft; red borders show **conflicts** when the same teacher, class, or **assigned room/lab** has **overlapping times**. Separate from that: a teacher teaching the **same subject** to the same class twice in one day must be one **continuous** multi-period block (otherwise it is flagged); returning later that day with a **different** subject needs at least the configured free-period gap (Timetable **Settings** → Teacher return rules; default 1 period). Timetable **Settings** also set teacher **workload limits**: max periods per day (default 6), max consecutive periods without a break (default 4), and whether teaching through break/lunch is an error (default) or warning. Use **Conflicts** tab or **Timetable Conflicts** in the sidebar for the full Conflict Resolution Centre (`/dashboard/headteacher/timetable/conflicts`). If the timetable is already **published** (no draft rows), the centre still scans the published schedule and shows conflicts — click **Create editable draft** there (or **Load draft** on the Edit tab) before applying fixes.
...
7. Fix grid conflicts by dragging lessons, removing bad entries, editing allocations, or using suggested fixes on the conflicts page. The Conflict Resolution Centre offers **Move here** buttons for specific free slots (teacher/class/room availability checked — not a generic tip alone). Numbers on the **Conflicts** tab, sidebar badge, control bar, grid header, and Conflict Resolution Centre all come from the **same server audit** (hard errors). Cell clash outlines during drag are local preview only and are not a separate count. Teacher colours are unique per school (aSc-style): each teacher has one stored colour, checked with CIEDE2000 so chips stay distinguishable at Class-wall size (practical school capacity ~**29** teachers under the current gate — enough for a typical secondary; larger staff may need admin colour curation). Manage under **Teacher colour codes**, and use the colour legend under each grid. Export **curriculum period compliance** (required vs scheduled periods — same math as Missing periods) as a Word file from the Conflict Centre. From **Teachers** / **One class** views (and My Timetable), download a single week as Word or open **Print / PDF**.
8. Assign **teacher colours** on the **Settings** tab (or auto-assign) so grids are easy to read.
9. Click **Publish** when there are **no hard conflicts** — teachers and students then see the published version. Publishing again for the same term **replaces** the previous published timetable (it does not stack a second copy). The server **blocks** publish if the draft is much smaller than the current published set (common mistake after generating only new departments) unless you intentionally force it. Dragging a lesson onto a slot that already overlaps another lesson for that class, teacher, or **room/lab** (when a venue is assigned) is **blocked immediately** (database rule), not only listed later on the Conflicts/audit screens. On publish (and on republish after editing a published timetable via draft), ZSMS **SMS-notifies only teachers whose personal schedule actually changed** (diff of previous vs new published periods) and asks them to check the app — teachers with identical slots are not messaged. SMS uses the school’s Africa’s Talking / outbound SMS credits and each teacher’s `contact_number`.
...
1. Navigate directly to `/dashboard/sms` (headteacher / admin).
2. Review the **Credits** card: remaining balance, messages used, lifetime granted, and (for trial schools) trial-pack usage (e.g. _X of 50 used_).
3. Enter phone numbers (Zambian format: `097…` auto-normalizes to `+26097…`) and message. You can load parent contacts from the page when available.
4. Send — each recipient deducts **one credit**. Send is disabled when the balance is 0.
5. If credits are exhausted, use **Subscribe to continue** (goes to `/pricing`) to keep messaging parents and staff.
6. Configure low-balance alert email and parent attendance SMS toggles (absent / late / present / excused) on the same page.

**Trial schools:** 50 credits are granted automatically at portal creation (once per school). They are not auto-renewed when the trial ends — subscribe for continued SMS capacity.
...
Trial schools receive **50 complimentary SMS credits** at onboarding. SMS broadcast remains plan-gated (Trial / Standard / Premium). When the balance hits 0, `/dashboard/sms` blocks send and links to `/pricing`. Paid plans still need a positive credit balance to send. AI tools require an AI-enabled plan slug.
