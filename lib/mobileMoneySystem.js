...
export class MobileMoneySystem {
  ...
  loadFeeStructure() {
    ...
    // Secondary school fees (Forms 1–6 / Grades 10–12; grade9 key kept for legacy fee rows)
    this.feeStructure.set('secondary', {
      ...
      examination: {
        grade9: 100, // legacy junior secondary exam fee key
  ...
  async processPayment(paymentData) {
    const { phoneNumber, amount, feeType, studentId, reference, provider = null } = paymentData
  ...
  async setupInstallmentPlan(installmentData) {
    const { studentId, totalAmount, numberOfInstallments, feeType, phoneNumber, startDate } =
      installmentData
    ...
    console.log(
      `Installment plan created: ${numberOfInstallments} payments of K${installmentAmount}`
    )
  ...
  async contributeToCommunityFund(contributionData) {
    const { phoneNumber, amount, fundType, contributorName, anonymous = false } = contributionData
  ...
  async checkFeeBalance(studentId) {
    ...
    if (!fees) {
      // Return zero balance if fee structure is not loaded
      return {
        studentId,
        totalDue: 0,
        totalPaid: 0,
  ...
  generatePaymentReport(period = 'monthly') {
    ...
    report.averageTransaction =
      report.totalTransactions > 0 ? report.totalAmount / report.totalTransactions : 0
  ...
  async callProviderAPI(provider, action, data) {
    ...
    if (action === 'payment') {
      const { sessionFetch } = await import('@/lib/auth/sessionFetch')
      const response = await sessionFetch('/api/payments/mobile-money', {
        method: 'POST',
      ...
      if (!response.ok) {
        throw new Error(resData?.error || `Payment gateway error: ${response.status}`)
      }

      return {
        transactionId: resData?.referenceId || data.reference,
        gateway: resData?.provider || 'lipila',
  ...
  async sendPaymentConfirmation(transaction) {
    ...
    if (window.smsSystem) {
      await window.smsSystem.sendSMS(
  ...
  getMobileMoneyStats() {
      ...
      providerStats[code] = {
        ...
        marketShare:
          completedTransactions.length > 0
            ? ((providerTransactions.length / completedTransactions.length) * 100).toFixed(1)
            : 0,
      }
    }
    ...
    return {
      ...
      totalFees,
      averageTransaction:
        completedTransactions.length > 0 ? totalAmount / completedTransactions.length : 0,
      successRate:
        this.transactionHistory.length > 0
          ? ((completedTransactions.length / this.transactionHistory.length) * 100).toFixed(1)
          : 0,
      providerStats,
      communityFunds: Object.fromEntries(this.communityFunds),
      activeLoans: Array.from(this.microLending.values()).filter((l) => l.status === 'active')
        .length,
    }
  }
  ...
  getTermStartDate() {
    ...
    switch (term) {
      case 1:
        return new Date(year, 0, 15) // January 15
      case 2:
