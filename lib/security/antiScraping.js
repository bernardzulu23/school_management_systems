...
export const SESSION_AUTH_PATHS = [
  '/api/auth/me',
  '/api/auth/refresh',
  '/api/auth/logout',
...
/** Modern browsers tag same-tab fetch() with Sec-Fetch-Site — trust that for cookie sessions. */
function isSameOriginBrowserFetch(request) {
...
function isTrustedBrowserSession(request) {
  if (hasBrowserClientHeaders(request)) return true
  return isSameOriginBrowserFetch(request)
}
...
export function checkAntiScraping(request, pathname, options = {}) {
  ...
  // React Native Android uses okhttp; desktop uses custom UAs. Both authenticate with Bearer.
  // Blocked-UA list targets anonymous scrapers — not holders of a school access token.
  if (isBlockedUserAgent(userAgent) && !bearer) {
    return { blocked: true, status: 403, code: 'blocked_client' }
  }

  // Cookie-session API calls must originate from the in-app browser (not curl/scripts).
  // Accept X-Requested-With + Accept, or Sec-Fetch-Site same-origin/same-site.
