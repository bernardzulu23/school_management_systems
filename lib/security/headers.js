...
export function buildContentSecurityPolicy(options = {}) {
  ...
  const styleSrc = ["'self'"]
  if (nonce) {
    // Nonces apply to <style>/<link>, NOT React `style={{…}}` attributes.
    // Timetable grids (AscClassWallGrid) and other UI rely on inline style props,
    // so 'unsafe-inline' is required alongside the nonce for styles.
    styleSrc.push(`'nonce-${nonce}'`, "'unsafe-inline'")
  } else {
    styleSrc.push("'unsafe-inline'")
  }
  styleSrc.push('https://fonts.googleapis.com', 'https://cdn.jsdelivr.net')
