...
/**
 * Build CSP. When `nonce` is set, script-src uses nonce + strict-dynamic (no unsafe-inline).
 * @param {{ nonce?: string, allowEval?: boolean, reportUri?: string }} [options]
 */
export function buildContentSecurityPolicy(options = {}) {
  ...
  scriptSrc.push('https://static.cloudflareinsights.com', 'https://challenges.cloudflare.com')
...
export function applySecurityHeaders(response, request, options = {}) {
  ...
  const nonce = options.nonce || request?.headers?.get?.('x-nonce') || generateNonce()
  ...
  // Nonced HTML must not be cached at the edge — each response needs a fresh nonce.
  const isDocument =
    pathname &&
