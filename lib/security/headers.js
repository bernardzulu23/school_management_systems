...
export function buildContentSecurityPolicy(options = {}) {
  ...
  // CSP3: a nonce/hash in style-src-elem disables 'unsafe-inline' for <style> tags.
  // Third-party UI (react-hot-toast) injects un-nonced <style> elements; without
  // unsafe-inline those styles are blocked and the toast host can sit as an
  // invisible full-viewport layer that intercepts clicks (including the sidebar).
  // Keep script nonces strict; allow inline styles for style elements + attributes.
  const styleSrcElem = ["'self'", "'unsafe-inline'", ...styleHosts]
...
export function applySecurityHeaders(response, request, options = {}) {
  ...
  if (isDocument && request?.method === 'GET') {
    if (isPublicEdgeCachePath(pathname)) {
      ...
      // Nonced HTML for authenticated/dynamic routes — do not cache at the edge (WCD).
