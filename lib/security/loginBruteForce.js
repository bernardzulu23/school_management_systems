...
import { recordFailedLoginAlert } from '@/lib/observability/alerts'
...
export function recordLoginFailure({ email, schoolId, ip }) {
  ...
  try {
    const ipHash = ip ? createHash('sha256').update(String(ip)).digest('hex').slice(0, 12) : null
...
export function handleLoginFailure({ request, email, schoolId, ip }) {
  const result = recordLoginFailure({ email, schoolId, ip })
  const lock = checkLoginBruteForce({ request, email, schoolId, ip })
  return {
    ...lock,
