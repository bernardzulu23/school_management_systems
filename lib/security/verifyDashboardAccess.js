...
async function userHasGuidanceAssignment(userId, schoolId) {
  if (!userId) return false
  const where = {
    userId: String(userId),
    active: true,
  ...
  const row = await prisma.guidanceAssignment.findFirst({
    where,
    select: { id: true },
  })
  return Boolean(row)
...
async function userHasSeniorTeacherAssignment(userId, schoolId) {
  if (!userId) return false
  const row = await prisma.seniorTeacherAssignment.findFirst({
    where: {
      userId: String(userId),
      active: true,
      revokedAt: null,
...
export async function requireDashboardRole(pathPrefix) {
  ...
  if (
    roleMatchesDashboardGroups(payload.role, gate.groups, {
  ...
  // Guidance portal: active GuidanceAssignment while User.role stayed "teacher".
  if (
    gate.groups.includes('GUIDANCE_TEACHER') &&
    (await userHasGuidanceAssignment(payload.id, payload.schoolId))
  ) {
    return
  }

  if (
    gate.groups.includes('SENIOR_TEACHER') &&
    (await userHasSeniorTeacherAssignment(payload.id, payload.schoolId))
  ) {
    return
  }
