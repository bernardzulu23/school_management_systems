...
const DASHBOARD_ROLE_GATES = [
  ...
  { prefix: '/dashboard/senior-teacher', groups: ['ADMIN', 'SENIOR_TEACHER'] },
 ...
 * @param {string[]} groups
 * @param {{ isHod?: boolean, isGuidance?: boolean } | null} [claims]
 */
export function roleMatchesDashboardGroups(role, groups, claims = null) {
  const roleKey = normalizeRoleKey(role)
  // Teachers who hold a HeadOfDepartment profile get isHod on the access token.
  if (claims?.isHod && groups.includes('HOD')) return true
  // Teachers with an active GuidanceAssignment get isGuidance on the access token.
  if (claims?.isGuidance && groups.includes('GUIDANCE_TEACHER')) return true
  if (claims?.isSeniorTeacher && groups.includes('SENIOR_TEACHER')) return true
