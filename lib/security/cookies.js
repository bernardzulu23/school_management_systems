...
/** Refresh tokens are scoped to the refresh endpoint only (not sent on every request). */
export function refreshTokenCookieOptions(request, { maxAgeSeconds }) {
  return {
    ...authCookieOptions(request, { maxAgeSeconds, name: 'refresh_token' }),
...
export function clearAuthSessionCookies(response, request) {
  for (const name of ['access_token', 'session-token', 'session', 'session_activity']) {
