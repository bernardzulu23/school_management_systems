  ...
  return dataRows
    .map((row, i) => {
      ...
      headers.forEach((header, colIdx) => {
        ...
        if (field) {
          obj[field] = normalizeCellValue(row[colIdx])
        }
      })
      return obj
    })
...
export function buildStudentUploadWorkbook({ title = 'ZSMS Student Bulk Upload' } = {}) {
  ...
  const ws = addAoaSheet(wb, 'Student Data', sheetData, {
    colWidths: [22, 24, 12, 28, 22, 16, 18, 14, 48, 22, 16],
  })
  // Father's Contact — Text so Excel does not strip leading zeros
