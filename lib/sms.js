...
export function buildTermResultsCompleteSmsMessage({
  ...
  if (safeLoginUrl) {
    return (
      `Dear Parent/Guardian, the Term results for ${safeStudent} have been entered. ` +
  ...
  if (!email) {
    return `Dear Parent/Guardian, all results for ${safeStudent} have been entered. Ask your child for login details or contact the school.${resetPart} - ${safeSchool}`
  }

  return `Dear Parent/Guardian, all results for ${safeStudent} have been entered. Sign in with student email: ${email} and password from your child.${resetPart} - ${safeSchool}`
}
