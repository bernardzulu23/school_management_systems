...
export function getSchoolSmsFrom(_school = null) {
  const from = String(
    process.env.AFRICAS_TALKING_SENDER_ID || process.env.AFRICASTALKING_SENDER_ID || ''
  ).trim()
  return from || null
}
...
/**
 * Prepend school name to free-text admin/headteacher SMS so parents see the source school.
 * Skips if the message already starts with the school name (case-insensitive).
 */
export function prefixSchoolNameToSms(schoolName, message) {
  ...
  if (lower.startsWith(schoolLower)) return text
...
export function formatResultGradeLine({ subjectName, score }, { abbreviate = false } = {}) {
  ...
  // Parent SMS: scores only (no letter/CBC grade labels).
