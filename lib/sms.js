...
export function buildTermResultsCompleteSmsMessage({
  ...
  const safeStudent = String(studentName || 'the student').trim() || 'the student'
  const safePossessive = String(possessive || 'their').trim() || 'their'
  const safeUsername = String(username || '').trim()
  const safeLoginUrl = String(loginUrl || '').trim()
  const safeResetUrl = String(resetUrl || '').trim()

  const portalPart = safeLoginUrl ? ` Portal: ${safeLoginUrl}.` : ''

  if (!safeUsername) {
    return `Dear Parent/Guardian, the academic results for ${safeStudent} are now available.${portalPart} Please contact the school for login access. Thank you.`
  }

  const passwordPart = safeResetUrl
    ? `Password: reset/set here: ${safeResetUrl}.`
    : `Password: use the one you already have for the portal.`

  return `Dear Parent/Guardian, the academic results for ${safeStudent} are now available.${portalPart} You may view them by logging into the school portal using ${safePossessive} login details: Username: ${safeUsername}. ${passwordPart} Thank you.`
}
