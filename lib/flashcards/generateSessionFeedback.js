...
import {
  FlashcardSessionFeedbackGenerationSchema,
  parseFlashcardSessionFeedback,
} from '@/lib/ai/schemas'
...
const FEEDBACK_SYSTEM =
  ...
  'Return JSON only with keys: summary (string), topicsToImprove (string[]), strengths (string[]), ' +
  'readingList (array of {type:"book"|"article", title, author?, reason}).'
...
/**
 * @param {{ subjectName: string, deckTitle?: string, score: ReturnType<import('./scoreSession').scoreFlashcardSession>, skipAI?: boolean }} params
 * @returns {Promise<{ feedback: object, fromAI: boolean }>}
 */
export async function generateFlashcardSessionFeedback({
  subjectName,
  deckTitle,
  score,
  skipAI = false,
  ...
  if (skipAI) {
    return { feedback: fallback, fromAI: false }
  }
  ...
  try {
    const { object } = await generateAIObject(
      ...
      prompt,
      {
        ...
        repair: (raw) => {
          const parsed = parseFlashcardSessionFeedback(raw)
          return parsed.success ? parsed.data : raw
        },
      }
    )

    const normalized = parseFlashcardSessionFeedback(object)
    if (normalized.success) {
      return { feedback: normalized.data, fromAI: true }
    }
    return { feedback: fallback, fromAI: false }
  } catch {
    return { feedback: fallback, fromAI: false }
