...
function resolveSsl(connectionString) {
  try {
    ...
    if (host.endsWith('render.com') && host.includes('postgres')) {
      return { rejectUnauthorized: false }
    }
    ...
    if (raw.includes('proxy.rlwy.net')) return { rejectUnauthorized: false }
    if (raw.includes('railway.internal')) return false
    if (raw.includes('postgres.render.com')) return { rejectUnauthorized: false }
    if (/localhost|127\.0\.0\.1/i.test(raw)) return false
    if (/(^|\/\/)db(?=[:/]|$)/i.test(raw)) return false
    if (/sslmode=(require|verify-ca|verify-full|no-verify)|ssl=true|ssl=1/i.test(raw))
      return { rejectUnauthorized: false }
