...
const DEFAULT_PROJECT_ID = 'prj_9fHza1PlabDZJuxYCoGBGOH9IaIq'
const DEFAULT_TEAM_ID = 'team_KowipjQTzEoEVBFNWM8vqxDA'

function analyticsConfig() {
  const token = String(process.env.VERCEL_API_TOKEN || '').trim()
  const projectId = String(process.env.VERCEL_PROJECT_ID || DEFAULT_PROJECT_ID).trim()
  const teamId = String(process.env.VERCEL_TEAM_ID || DEFAULT_TEAM_ID).trim()
