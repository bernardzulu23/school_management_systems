...
export async function getPlatformHealthChecks() {
  ...
  checks.push({
    id: 'groq',
    label: 'Groq API (primary AI)',
    status: hasGroq ? 'ok' : hasGemini ? 'warn' : 'fail',
    hint: !hasGroq
      ? hasGemini
        ? 'Groq unset — Gemini fallback only (set GROQ_API_KEY for primary)'
  ...
  checks.push({
    id: 'gemini',
    label: 'Gemini API (AI fallback)',
    status: hasGemini ? 'ok' : hasGroq ? 'warn' : 'fail',
    hint: !hasGemini
      ? hasGroq
        ? 'Optional: set GEMINI_API_KEY from aistudio.google.com for failover'
        : 'Set GEMINI_API_KEY for free fallback when Groq is unavailable'
      : undefined,
    doc: '/docs/DEVELOPER_GUIDE.md',
  })
