...
/**
 * Platform-admin totals — derived from per-school groupBy (never unscoped table scans).
 * Do not call from public routes; use getPlatformSchoolUsageStats for dashboards.
 */
export async function getPlatformUsageTotals() {
  const usage = await getPlatformSchoolUsageStats({ includeInactive: false })
  return {
    students: usage.totals.students,
    teachers: usage.totals.teachers,
    activeSchools: usage.totals.schools,
  }
}
