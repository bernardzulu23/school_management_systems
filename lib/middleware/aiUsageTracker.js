...
/**
 * Plan used for AI quotas. Active trial / pilot extensions get premium limits
 * (same as displayPlanForSchool) so multi-month pilots are not stuck at 10/day.
 */
export function getEffectivePlanForAiLimits(school) {
  const sub = getSubscriptionState(school)
  if (!sub.active) return 'unpaid'
  if (sub.onTrial || sub.isTrialPlan) return 'premium'
  return normalizePlan(school?.plan)
}

/** @deprecated Use getDailyLimit — kept for callers that still reference monthly naming */
export function getMonthlyLimit(plan) {
  return getDailyLimit(plan)
}
...
export async function checkAILimit(schoolId, userId = null) {
  ...
  // Must match dashboard / requireFeature — do not treat stale planExpiresAt as
  // expired when trialEndsAt was extended (pilot) or is still valid.
  ...
  const limit = getDailyLimit(school.plan)
  if (!Number.isFinite(limit)) return null

  const dayKey = getDayKey()
  const used = await getDailyUsageCount(school.id, dayKey)
  if (used >= limit) {
    return NextResponse.json(
      {
