...
export async function enforceSubscriptionIfNeeded(request) {
  ...
  // #region agent log
  try {
    const { agentDebugLog } = await import('@/lib/debug/agentLog')
    agentDebugLog({
      hypothesisId: 'B',
      location: 'subscriptionGate.js:blocked',
      message: 'subscription blocked API',
      data: {
        pathname,
        plan: sub.plan,
        expired: sub.expired,
        ...
        schoolId: String(school.id || '').slice(0, 8),
      },
    })
  } catch {
    /* ignore */
