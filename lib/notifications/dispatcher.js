...
import { sendExpoPushToUser } from '@/lib/notifications/expoPushService'
import {
...
async function deliverChannel(channel, ctx) {
  const { user, schoolId, schoolName, title, message, actionUrl } = ctx
...
export async function retryNotificationChannel(notification, channel) {
  const schoolId = notification.schoolId ? String(notification.schoolId) : null
  const user = await prisma.user.findFirst({
    where: {
