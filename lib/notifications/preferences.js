...
export function filterChannelsByPreference(channels, prefs) {
  ...
  for (const ch of channels) {
    ...
    if (ch === 'EXPO_PUSH' && prefs.expoPushEnabled !== false) enabled.push(ch)
  ...
  if (enabled.length === 0) {
    ...
    else if (prefs.expoPushEnabled !== false) enabled.push('EXPO_PUSH')
