...
function buildUserPrompt({
  ...
  const modeHint =
    assessmentMode === ASSESSMENT_MODES.SECONDARY_SCENARIO
      ? 'Use secondary ECZ scenario style: each item in questions[] must include zambianScenario + subQuestions with marks. No bare MCQ/true-false.'
  ...
  return [
    ...
    schemeBlock,
    '',
    'Respond with JSON shaped as: { "title": string, "questions": [ ... ] }.',
    'Do not use a top-level "scenarios" key — put scenario items inside questions[].',
  ...
  while (validated.length < target && attempts < maxAttempts) {
    ...
    try {
      ;({ object } = await generateAIObject(
        ...
        { temperature: 0.65, maxOutputTokens: 4500, repair: coerceQuizObject, maxRetries: 3 }
      ))
    } catch {
      rejectedCount += 1
      continue
      ...
      if (!validation.valid) {
        ...
        try {
          ...
          const { object: repairedObj } = await generateAIObject(
            ...
            SYSTEM,
            repairPrompt,
            { temperature: 0.4, maxOutputTokens: 2500, repair: coerceQuizObject, maxRetries: 2 }
          )
