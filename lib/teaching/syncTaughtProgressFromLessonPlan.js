...
export async function syncTaughtProgressFromLessonPlan({ lessonPlanId, taught }) {
  ...
  if (scheme) {
    ...
    if (taught) {
      const fullScheme = await prisma.schemeOfWork.findUnique({
        where: { id: schemeId },
        select: { weeks: true, subject: true, gradeOrForm: true },
      })
      const weekRows = weeksFromSchemeJson(fullScheme?.weeks)
      const totalWeeks = weekRows.length || 12
      const completedWeeks = await prisma.schemeProgress.count({
        where: { schemeId, completed: true },
      })
      const { notifySchemeProgressUpdate } = await import('@/lib/notifications/integrations')
      await notifySchemeProgressUpdate({
        schoolId: scheme.schoolId,
        teacherId: scheme.teacherId,
        subject: fullScheme?.subject,
        gradeOrForm: fullScheme?.gradeOrForm,
        completedWeeks,
        totalWeeks,
        weekNumber,
        topicName,
