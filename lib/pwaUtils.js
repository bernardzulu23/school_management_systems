...
export class PWAInstaller {
  ...
  init() {
    ...
    this._unsub = subscribePwaInstall(({ installable, installed }) => {
      this.isInstallable = Boolean(installable)
      this.isInstalled = Boolean(installed)
      this.deferredPrompt = getDeferredInstallPrompt()
      if (this.isInstallable) this.onInstallable()
      if (this.isInstalled) this.onInstalled()
    })
  }
  ...
  async showInstallPrompt() {
    ...
    this.deferredPrompt = getDeferredInstallPrompt()
    this.isInstallable = Boolean(this.deferredPrompt)
    return accepted
  }
...
export class NotificationManager {
  ...
  async requestPermission() {
    ...
    // Already decided — never re-prompt (avoids heuristic "excessive requests").
    ...
    try {
      // Caller must invoke from a user gesture when permission is still "default".
