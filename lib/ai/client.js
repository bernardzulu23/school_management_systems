...
export const GROQ_MODEL_EXPORT = GROQ_MODEL
/** @deprecated alias — use GROQ_STREAM_MODEL */
export const GROQ_STREAM_MODEL_EXPORT = GROQ_STREAM_MODEL
/** @deprecated alias — use GROQ_STRUCTURED_MODEL */
export const GROQ_STRUCTURED_MODEL_EXPORT = GROQ_STRUCTURED_MODEL

export { GROQ_MODEL, GROQ_STREAM_MODEL, GROQ_STRUCTURED_MODEL, GROQ_FALLBACK_MODELS, groqModelFor }
 ...
 * @param {{ maxOutputTokens?: number, maxTokens?: number, temperature?: number, maxRetries?: number, models?: string[], repair?: (raw: unknown) => unknown, captureFailure?: boolean }} [options]
 */
export async function generateAIObject(schema, systemPrompt, userPrompt, options = {}) {
  assertGroqConfigured()
  const log = logger({ route: 'AI:generateObject' })
  const captureFailure = options.captureFailure !== false
  const models =
    options.models ||
    [GROQ_STRUCTURED_MODEL, ...GROQ_FALLBACK_MODELS].filter((m, i, a) => a.indexOf(m) === i)
  ...
  try {
    ...
    if (raw) {
      ...
      if (typeof options.repair === 'function') {
        try {
          candidate = options.repair(raw) ?? raw
        } catch {
          candidate = raw
        }
      }
      let parsed = schema.safeParse(candidate)
      if (!parsed.success && candidate !== raw) {
        parsed = schema.safeParse(raw)
      }
      if (!parsed.success && typeof options.repair === 'function') {
        try {
          parsed = schema.safeParse(options.repair(raw))
        } catch {
          /* keep failure */
        }
      }
      ...
      log.warn('Provider chain JSON schema mismatch', {
        provider: chainResult.provider,
  ...
  if (captureFailure) {
    ...
    log.warn('AI object generation soft-failed (no Sentry capture)', {
      message: lastError?.message,
    })
  }
  throw new Error(
    `AI generation failed: ${lastError?.message || 'unknown error'}. All providers were tried (Groq structured → full chain). Set GROQ_STRUCTURED_MODEL to llama-3.1-8b-instant if Groq keeps failing.`
  )
}
