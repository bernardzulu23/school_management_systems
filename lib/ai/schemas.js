/**
 * Zod schemas for all AI-generated content in ZSMS.
 *
 * Used with generateAIObject() — outputs are validated before returning to routes.
 *
 * @see docs/AI_GUIDE.md
 */
import { z } from 'zod'

// ─── ECZ RUBRIC ──────────────────────────────────────────────────────────────

export const RubricCriterionSchema = z.object({
  name: z.string().min(3).max(100).describe('Name of the assessment criterion'),
  description: z.string().max(200).describe('What this criterion measures'),
  excellent: z.string().min(10).describe('Descriptor for Excellent (4 marks)'),
  good: z.string().min(10).describe('Descriptor for Good (3 marks)'),
  fair: z.string().min(10).describe('Descriptor for Fair (2 marks)'),
  needsImprovement: z.string().min(10).describe('Descriptor for Needs Improvement (1 mark)'),
})

export const RubricSchema = z.object({
  taskTitle: z.string(),
  subject: z.string(),
  form: z.enum(['Form 1', 'Form 2', 'Form 3']),
  taskType: z.enum([
    'Project',
    'Practical task',
    'Assignment',
    'Presentation',
    'Fieldwork',
    'Portfolio',
    'Observation',
    'End of term test',
  ]),
  criteria: z.array(RubricCriterionSchema).min(2).max(8),
  zambiaCurriculumAlignment: z.string().describe('Which ECZ competencies this task develops'),
})

// ─── LESSON PLAN ──────────────────────────────────────────────────────────────

export const LessonObjectiveSchema = z.object({
  objective: z.string().min(10),
  bloomsLevel: z.enum([
    'Remembering',
    'Understanding',
    'Applying',
    'Analysing',
    'Evaluating',
    'Creating',
  ]),
  competency: z.string(),
})

export const LessonActivitySchema = z.object({
  phase: z.enum(['Introduction', 'Development', 'Conclusion']),
  durationMinutes: z.number().min(2).max(60),
  activity: z.string().min(20),
  teacherAction: z.string(),
  learnerAction: z.string(),
  resources: z.array(z.string()),
  zambiaCulturalContext: z.string().optional(),
})

export const LessonPlanSchema = z.object({
  title: z.string(),
  subject: z.string(),
  gradeOrForm: z.string(),
  duration: z.number().describe('Total duration in minutes'),
  objectives: z.array(LessonObjectiveSchema).min(1).max(5),
  priorKnowledge: z.string(),
  materialsRequired: z.array(z.string()),
  activities: z.array(LessonActivitySchema).min(2).max(8),
  assessment: z.object({
    method: z.string(),
    tool: z.string(),
    criteria: z.string(),
  }),
  crossCuttingThemes: z.array(z.string()),
  coreCompetencies: z.array(z.string()).min(1).max(3),
  realWorldZambianContext: z.string(),
  teacherReflectionPrompts: z.array(z.string()).max(3),
})

// ─── ECZ EXAM QUESTION ───────────────────────────────────────────────────────

export const ECZSubQuestionSchema = z.object({
  number: z.string().describe('e.g. (a), (b), (i), (ii)'),
  commandTerm: z.enum([
    'State',
    'List',
    'Define',
    'Describe',
    'Explain',
    'Calculate',
    'Compare',
    'Contrast',
    'Analyse',
    'Evaluate',
    'Design',
    'Justify',
    'Synthesise',
    'Discuss',
    'Identify',
    'Outline',
    'Summarise',
  ]),
  question: z.string(),
  marks: z.number().min(1).max(20),
  bloomsLevel: z.enum([
    'Remembering',
    'Understanding',
    'Applying',
    'Analysing',
    'Evaluating',
    'Creating',
  ]),
  modelAnswer: z.string().describe('Mark scheme / model answer'),
})

export const ECZExamQuestionSchema = z.object({
  questionNumber: z.number(),
  zambianScenario: z.string().min(30).describe('Real-life Zambian context (2-4 sentences)'),
  subject: z.string(),
  form: z.enum(['Form 1', 'Form 2', 'Form 3', 'Form 4']),
  elementOfConstruct: z.string().describe('Which ECZ element of construct this assesses'),
  subQuestions: z.array(ECZSubQuestionSchema).min(2).max(6),
  totalMarks: z.number(),
  hasMultipleChoice: z.literal(false).describe('NEVER true at secondary level'),
})

export const ECZExamQuestionsResponseSchema = z.object({
  scenarios: z.array(ECZExamQuestionSchema).min(1).max(5),
})

// ─── SBA TASK ────────────────────────────────────────────────────────────────

export const SBATaskSchema = z.object({
  title: z.string(),
  subject: z.string(),
  form: z.enum(['Form 1', 'Form 2', 'Form 3']),
  taskType: z.string(),
  context: z.string().min(30).describe('Real-life Zambian scenario'),
  task: z.string().min(30).describe('Clear instructions for learner'),
  materialsProvided: z.array(z.string()),
  rubric: RubricSchema,
  demonstration: z.string().describe('What learner must show after completing task'),
  competencies: z.array(z.string()).min(1),
  crossCuttingThemes: z.array(z.string()),
  estimatedDurationMinutes: z.number(),
  resourceLevel: z.enum(['low', 'moderate', 'well-resourced']),
})

// ─── REPORT COMMENT ──────────────────────────────────────────────────────────

export const ReportCommentSchema = z.object({
  subject: z.string(),
  studentName: z.string(),
  termComment: z.string().min(30).max(300).describe('Professional end-of-term comment'),
  strengths: z.array(z.string()).max(3),
  areasForGrowth: z.array(z.string()).max(2),
  recommendedActions: z.string().max(200),
  cbcCompetenciesEvidenced: z.array(z.string()),
})

// ─── QUIZ (formative) ────────────────────────────────────────────────────────

function normalizeQuizType(raw) {
  const t = String(raw || '')
    .toLowerCase()
    .trim()
  if (t.includes('true') || t === 'tf' || t === 't/f') return 'true_false'
  if (
    t.includes('short') ||
    t.includes('open') ||
    t.includes('written') ||
    t.includes('structured') ||
    t.includes('scenario') ||
    t.includes('extended') ||
    t.includes('calculation') ||
    t.includes('essay')
  ) {
    return 'short'
  }
  if (t.includes('mcq') || t.includes('multiple') || t === 'choice') return 'mcq'
  // Default formative shape
  return 'mcq'
}

function normalizeOptionList(options) {
  if (typeof options === 'string') {
    return options
      .split(/\n|;|\|/)
      .map((s) => s.trim())
      .filter(Boolean)
  }
  if (!Array.isArray(options)) return undefined
  return options
    .map((o) => {
      if (typeof o === 'string' || typeof o === 'number' || typeof o === 'boolean') return String(o)
      if (o && typeof o === 'object') {
        return String(o.text ?? o.label ?? o.option ?? o.value ?? o.answer ?? '')
      }
      return ''
    })
    .map((s) => s.trim())
    .filter(Boolean)
}

function normalizeAnswer(value) {
  if (value == null) return ''
  if (typeof value === 'boolean' || typeof value === 'number') return String(value)
  if (typeof value === 'object') {
    return String(value.text ?? value.label ?? value.answer ?? value.value ?? '')
  }
  return String(value)
}

/**
 * Repair common Groq / free-model quiz JSON shape issues before Zod.
 * Handles `{ scenarios: [...] }` (secondary ECZ style), option objects, boolean answers, etc.
 */
export function coerceQuizObject(raw) {
  if (Array.isArray(raw)) return coerceQuizObject({ questions: raw })
  if (!raw || typeof raw !== 'object') return raw

  const src =
    (raw.quiz && typeof raw.quiz === 'object' && raw.quiz) ||
    (raw.data && typeof raw.data === 'object' && raw.data) ||
    (raw.paper && typeof raw.paper === 'object' && raw.paper) ||
    raw

  const questionsIn = Array.isArray(src.questions)
    ? src.questions
    : Array.isArray(src.items)
      ? src.items
      : Array.isArray(src.scenarios)
        ? src.scenarios
        : Array.isArray(src.quiz?.questions)
          ? src.quiz.questions
          : []

  const questions = questionsIn.map((q, i) => {
    const row = q && typeof q === 'object' ? q : { question: String(q || '') }
    const options = normalizeOptionList(row.options)
    const scenario = row.zambianScenario || row.scenario || row.context
    const hasSubs = Array.isArray(row.subQuestions) && row.subQuestions.length > 0
    const type = normalizeQuizType(
      row.type || (hasSubs || scenario ? 'scenario' : options?.length ? 'mcq' : 'short')
    )
    const marks =
      row.marks != null && Number.isFinite(Number(row.marks))
        ? Number(row.marks)
        : row.totalMarks != null && Number.isFinite(Number(row.totalMarks))
          ? Number(row.totalMarks)
          : undefined

    const questionText = String(
      row.question || row.prompt || row.text || scenario || `Question ${i + 1}`
    ).trim()

    return {
      id: String(row.id ?? row.questionNumber ?? `q${i + 1}`),
      type: type === 'short' && (hasSubs || scenario) ? 'short' : type,
      question: questionText || `Question ${i + 1}`,
      options: type === 'mcq' ? (options?.length ? options : ['A', 'B', 'C', 'D']) : options,
      marks,
      answer: normalizeAnswer(row.answer ?? row.correctAnswer ?? row.correct ?? row.modelAnswer),
      explanation: row.explanation != null ? String(row.explanation) : undefined,
      // Preserve ECZ scenario fields for scheme-test / secondary validation.
      ...(scenario ? { zambianScenario: String(scenario) } : {}),
      ...(hasSubs ? { subQuestions: row.subQuestions } : {}),
      ...(row.bloomsLevel || row.bloomLevel
        ? { bloomsLevel: String(row.bloomsLevel || row.bloomLevel) }
        : {}),
      ...(row.commandTerm ? { commandTerm: String(row.commandTerm) } : {}),
      ...(row.elementOfConstruct ? { elementOfConstruct: String(row.elementOfConstruct) } : {}),
      ...(row.topic || row.topicTag ? { topic: String(row.topic || row.topicTag) } : {}),
    }
  })

  return {
    title: String(src.title || src.name || 'Quiz'),
    grade: String(src.grade || src.form || ''),
    subject: String(src.subject || ''),
    topic: String(src.topic || ''),
    totalMarks:
      src.totalMarks != null && Number.isFinite(Number(src.totalMarks))
        ? Number(src.totalMarks)
        : undefined,
    questions,
  }
}

export const QuizQuestionSchema = z
  .object({
    id: z.string().min(1),
    type: z.enum(['mcq', 'short', 'true_false']),
    question: z.string().min(1),
    options: z.array(z.string()).optional(),
    marks: z.number().optional(),
    answer: z.string(),
    explanation: z.string().optional(),
    zambianScenario: z.string().optional(),
    subQuestions: z.array(z.any()).optional(),
    bloomsLevel: z.string().optional(),
    commandTerm: z.string().optional(),
    elementOfConstruct: z.string().optional(),
    topic: z.string().optional(),
  })
  .passthrough()

/**
 * Loose schema for LLM generateObject (avoids brittle free-model JSON failures).
 * Accepts questions[], scenarios[], or items[] — coerceQuizObject normalizes afterward.
 */
export const QuizGenerationSchema = z
  .object({
    title: z.any().optional(),
    grade: z.any().optional(),
    form: z.any().optional(),
    subject: z.any().optional(),
    topic: z.any().optional(),
    totalMarks: z.any().optional(),
    questions: z.array(z.any()).optional(),
    scenarios: z.array(z.any()).optional(),
    items: z.array(z.any()).optional(),
    quiz: z.any().optional(),
    data: z.any().optional(),
    paper: z.any().optional(),
  })
  .passthrough()
  .superRefine((data, ctx) => {
    const coerced = coerceQuizObject(data)
    if (!Array.isArray(coerced?.questions) || coerced.questions.length < 1) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Expected at least one question (questions, scenarios, or items)',
        path: ['questions'],
      })
    }
  })

export const QuizSchema = z.object({
  title: z.string().min(1),
  grade: z.string(),
  subject: z.string(),
  topic: z.string(),
  totalMarks: z.number().optional(),
  questions: z.array(QuizQuestionSchema).min(1).max(30),
})

/** Parse model output into a validated quiz (coerce first). */
export function parseQuizObject(raw) {
  const coerced = coerceQuizObject(raw)
  return QuizSchema.safeParse(coerced)
}

export const QuizClassAnalysisSchema = z.object({
  summary: z.string(),
  classAverage: z.number(),
  strugglingTopics: z.array(z.string()).min(1).max(10),
  recommendReteach: z.boolean(),
  reteachRationale: z.string(),
  suggestedActivities: z.array(z.string()).min(1).max(8),
  studentsNeedingSupport: z.string(),
})

// ─── STUDENT FLASHCARDS (AI, self-quiz) ──────────────────────────────────────

export const FlashcardSchema = z.object({
  id: z.string().optional(),
  front: z.string().describe('Question or prompt shown to the student'),
  options: z.array(z.string()).min(2).max(6).describe('Answer choices'),
  answer: z.string().describe('The correct option, matching one of options exactly'),
  explanation: z.string().optional().describe('Short reason shown after the student selects'),
})

export const FlashcardDeckSchema = z.object({
  title: z.string().optional(),
  subject: z.string().optional(),
  topic: z.string().optional(),
  cards: z.array(FlashcardSchema).min(1).max(12),
})

export const FlashcardReadingItemSchema = z.object({
  type: z.preprocess(
    (value) => {
      const s = String(value || '')
        .trim()
        .toLowerCase()
      if (!s) return value
      if (s.includes('book') || s.includes('textbook') || s.includes('guide')) return 'book'
      if (
        s.includes('article') ||
        s.includes('paper') ||
        s.includes('web') ||
        s.includes('blog') ||
        s.includes('notes')
      ) {
        return 'article'
      }
      return s
    },
    z.enum(['book', 'article'])
  ),
  title: z.string().min(1),
  author: z.string().optional(),
  reason: z.string().optional().describe('Why the student should read this'),
})

/** Strict feedback shape returned to clients after coercion. */
export const FlashcardSessionFeedbackSchema = z.object({
  summary: z.string().min(1),
  topicsToImprove: z.array(z.string()).min(1).max(8),
  strengths: z.array(z.string()).min(1).max(6),
  readingList: z.array(FlashcardReadingItemSchema).min(1).max(6),
})

/**
 * Loose LLM schema — models often rename fields or invent reading types.
 * Prefer `parseFlashcardSessionFeedback` after generateObject / text JSON.
 */
export const FlashcardSessionFeedbackGenerationSchema = z
  .object({
    summary: z.any().optional(),
    topicsToImprove: z.any().optional(),
    topics: z.any().optional(),
    strengths: z.any().optional(),
    readingList: z.any().optional(),
    readings: z.any().optional(),
    resources: z.any().optional(),
  })
  .passthrough()

function asStringList(value, { min = 1, max = 8, fallback = ['Keep practising'] } = {}) {
  let list = []
  if (Array.isArray(value)) {
    list = value.map((v) => String(v ?? '').trim()).filter(Boolean)
  } else if (typeof value === 'string' && value.trim()) {
    list = value
      .split(/[\n;|]+/)
      .map((s) => s.trim())
      .filter(Boolean)
  }
  if (!list.length) list = [...fallback]
  return list.slice(0, max)
}

/**
 * Normalize messy model JSON into FlashcardSessionFeedbackSchema.
 * @param {unknown} raw
 * @returns {import('zod').SafeParseReturnType<any, any>}
 */
export function parseFlashcardSessionFeedback(raw) {
  const src = raw && typeof raw === 'object' && !Array.isArray(raw) ? raw : {}
  const readingRaw = Array.isArray(src.readingList)
    ? src.readingList
    : Array.isArray(src.readings)
      ? src.readings
      : Array.isArray(src.resources)
        ? src.resources
        : []

  const readingList = readingRaw
    .map((item) => {
      if (!item || typeof item !== 'object') return null
      const title = String(item.title || item.name || item.resource || '').trim()
      if (!title) return null
      return {
        type: item.type || item.kind || 'article',
        title,
        author: item.author != null ? String(item.author) : undefined,
        reason: String(
          item.reason || item.why || item.description || 'Helpful revision reading.'
        ).trim(),
      }
    })
    .filter(Boolean)

  const candidate = {
    summary: String(src.summary || src.feedback || src.message || '').trim(),
    topicsToImprove: asStringList(src.topicsToImprove ?? src.topics, {
      fallback: ['Review the questions you missed'],
    }),
    strengths: asStringList(src.strengths, {
      max: 6,
      fallback: ['Completed the flashcard deck'],
    }),
    readingList: readingList.length
      ? readingList.slice(0, 6)
      : [
          {
            type: 'article',
            title: 'CBC revision notes for this subject',
            author: 'ZSMS Study Library',
            reason: "Short reading to reinforce today's quiz topics.",
          },
        ],
  }

  if (!candidate.summary) {
    candidate.summary =
      'Thanks for finishing this deck. Review the topics below and try another practice round soon.'
  }

  return FlashcardSessionFeedbackSchema.safeParse(candidate)
}
// ─── ECZ PRACTICE PAPER (multi-level) ────────────────────────────────────────

export const ECZPracticeQuestionSchema = z.object({
  id: z.string(),
  type: z.enum(['mcq', 'short', 'structured']),
  question: z.string(),
  options: z.array(z.string()).optional(),
  marks: z.number(),
  answer: z.string(),
  explanation: z.string().optional(),
})

export const ECZPracticeScenarioSchema = z.object({
  questionNumber: z.number(),
  zambianScenario: z.string().min(30),
  elementOfConstruct: z.string().optional(),
  hasMultipleChoice: z.literal(false).optional(),
  subQuestions: z
    .array(
      z.object({
        number: z.string(),
        commandTerm: z.string(),
        question: z.string(),
        marks: z.number(),
        bloomsLevel: z.string().optional(),
        modelAnswer: z.string().optional(),
      })
    )
    .optional(),
  totalMarks: z.number().optional(),
})

export const ECZPracticePaperSchema = z.object({
  paper: z.object({
    examInfo: z.object({
      subject: z.string(),
      level: z.string(),
      topic: z.string(),
      totalMarks: z.number(),
      timeAllowed: z.string(),
    }),
    questions: z.array(ECZPracticeQuestionSchema).max(20).default([]),
    scenarios: z.array(ECZPracticeScenarioSchema).optional(),
  }),
})

// ─── SBA / INNOVATION PROJECT BRIEF (AI) ─────────────────────────────────────

export const ProjectRubricCriterionSchema = z.object({
  name: z.string().min(2).max(100),
  description: z.string().max(200).optional(),
  excellent: z.string().min(10),
  good: z.string().min(10),
  fair: z.string().min(10),
  needsImprovement: z.string().min(10),
})

/** Structured ECZ SBA project brief — used by /api/ai/project-maker */
export const ProjectGenerationSchema = z.object({
  title: z.string().min(5).max(200),
  subject: z.string(),
  form: z.string(),
  topic: z.string(),
  taskType: z.string().default('Project'),
  context: z
    .string()
    .min(30)
    .describe('Real-life Zambian scenario (locations, occupations, community)'),
  instructions: z
    .string()
    .min(40)
    .describe('Clear learner instructions combining steps and expectations'),
  steps: z.array(z.string().min(5)).min(3).max(10),
  deliverables: z.array(z.string().min(5)).min(2).max(8),
  timeline: z.string().min(10).describe('Checkpoints / estimated duration'),
  materials: z.array(z.string()).max(12),
  demonstration: z.string().min(20).describe('What the learner must show when done'),
  competencies: z.array(z.string()).min(1).max(5),
  criteria: z.array(ProjectRubricCriterionSchema).min(2).max(6),
})
