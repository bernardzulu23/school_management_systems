...
export function coerceQuizObject(raw) {
  ...
  const src = raw.quiz && typeof raw.quiz === 'object' ? raw.quiz : raw
  ...
  const questions = questionsIn.map((q, i) => {
    ...
    const type = normalizeQuizType(row.type || (options?.length ? 'mcq' : 'short'))
    const marks =
      row.marks != null && Number.isFinite(Number(row.marks)) ? Number(row.marks) : undefined

    return {
      id: String(row.id || `q${i + 1}`),
      type,
      question: String(row.question || row.prompt || row.text || `Question ${i + 1}`),
      options: type === 'mcq' ? (options?.length ? options : ['A', 'B', 'C', 'D']) : options,
      marks,
      answer: String(row.answer ?? row.correctAnswer ?? row.correct ?? ''),
