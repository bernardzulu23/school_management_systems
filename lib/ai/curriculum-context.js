...
const LOWER_PRIMARY_ZAMBIAN_LANGUAGE_ALIASES = new Set([
  'bemba',
  'icibemba',
  'chinyanja',
  'cinyanja',
...
function resolveGroundingSubject(subject, formOrGrade) {
  const raw = String(subject || '').trim()
  if (!raw) return raw
  const lower = raw.toLowerCase()
  const primaryGrade = toPrimaryGradeNumber(formOrGrade)
  if (lower === 'creative and technology studies' && primaryGrade && primaryGrade <= 3) {
    return 'Creative and Technology Studies'
  }
  if (primaryGrade && primaryGrade <= 3 && LOWER_PRIMARY_ZAMBIAN_LANGUAGE_ALIASES.has(lower)) {
    return 'Zambian Languages'
  }
...
export async function resolveCurriculumContext(subject, formOrGrade) {
  ...
  const subj = resolveGroundingSubject(requested, formOrGrade)
