 ...
 * When no topics exist, returns the trimmed topic unchanged (free-form allowed).
 *
 * @param {string | null | undefined} subject
 * @param {string | number | null | undefined} formOrGrade
...
export async function assertCurriculumTopicAllowed(subject, formOrGrade, topic, options = {}) {
  ...
  if (!requested) {
    if (options.required) {
      throw new Error('topic must be one of the curriculum topics for this subject')
    }
    return ''
  }
  ...
  throw new Error(
    `Topic "${requested}" is not in the curriculum for ${subject}. Choose a syllabus topic.`
  )
}
