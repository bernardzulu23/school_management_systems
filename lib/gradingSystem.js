...
/**
 * ECZ CBC Competence Based Curriculum grading for Forms 1–6.
 * Scores map to competency levels 1 (Outstanding) … 5 (Unsatisfactory).
 */
export const CBC_SECONDARY_GRADING_SYSTEM = {
  name: 'CBC Secondary Grading (Forms 1–6)',
  grades: [
    {
      min: 70,
      max: 100,
...
/** @deprecated Kept for Grade 10–12 / legacy aliases; Forms 1–6 use CBC. */
export const JUNIOR_GRADING_SYSTEM = CBC_SECONDARY_GRADING_SYSTEM

/** Legacy senior bands for Grade 10–12 class labels (not Forms 1–6). */
export const SENIOR_GRADING_SYSTEM = {
  name: 'Senior Grading (Grades 10-12)',
  grades: [
    {
      min: 75,
      max: 100,
...
const CBC_FORM_LEVEL_KEYS = new Set(['form1', 'form2', 'form3', 'form4', 'form5', 'form6'])
const JUNIOR_LEVEL_KEYS = CBC_FORM_LEVEL_KEYS
...
const GRADE_LABEL_COLORS = {
  ...
  FOUR: 'yellow',
  F: 'red',
  // CBC Forms 1–6 (and overlapping senior labels 1–4)
  1: 'green',
  2: 'blue',
  3: 'purple',
  4: 'yellow',
...
export function getGradingSystem(gradeLevel) {
  ...
  if (CBC_FORM_LEVEL_KEYS.has(key) || (normalized && CBC_FORM_LEVEL_KEYS.has(normalized))) {
    return CBC_SECONDARY_GRADING_SYSTEM
  }
  if (SENIOR_LEVEL_KEYS.has(key) || (normalized && SENIOR_LEVEL_KEYS.has(normalized))) {
    return SENIOR_GRADING_SYSTEM
  }

  // Unknown / primary-style labels: default to CBC secondary bands for numeric conversion
  return CBC_SECONDARY_GRADING_SYSTEM
}
...
export function gradeToPoints(grade, gradeLevel = null) {
  ...
  // Legacy senior fail
  if (n === 9) return null
  // CBC Forms 1–6: level 5 = Unsatisfactory (no points). Senior Grade 10–12: 5 = CREDIT.
  if (n === 5) {
    const normalized = normalizeGradeLevel(gradeLevel)
    if (normalized && SENIOR_LEVEL_KEYS.has(normalized)) return 5
    if (normalized && CBC_FORM_LEVEL_KEYS.has(normalized)) return null
    // No level context: treat as CBC unsatisfactory
    return null
  }
  if (n >= 1 && n <= 8) return n
  return null
...
export function scoreToPoints(score, gradeLevel) {
  ...
  return gradeToPoints(gradeInfo.grade, gradeLevel)
}
...
/** Top achievement grades: CBC Outstanding/Advanced (1–2) and legacy ONE/TWO. */
