...
const JUNIOR_LEVEL_KEYS = new Set(['form1', 'form2', 'grade9', 'grade 8', 'grade 9'])
...
export function normalizeGradeLevel(gradeLevel) {
  ...
  const gradeMatch =
    lower.match(/\bgrade\s*(1[0-2]|[89])\s*[a-z]?\b/) || lower.match(/^grade(1[0-2]|[89])[a-z]?$/)
  if (gradeMatch) {
  ...
  const compactMatch = lower.match(/^(1[0-2]|[1-9])[a-z]$/)
  if (compactMatch) {
    const n = Number(compactMatch[1])
    if (n >= 10) return 'grade10'
    if (n <= 2) return 'form1'
    if (n >= 3 && n <= 6) return 'form3'
    if (n >= 8) return 'grade9'
  }

  const leadingNum = lower.match(/^(1[0-2]|[1-9])\b/)
  if (leadingNum) {
  ...
  if (JUNIOR_LEVEL_KEYS.has(lower))
    return lower === 'grade 8' ? 'grade9' : lower.replace(/\s+/g, '')
