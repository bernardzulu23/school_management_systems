 ...
 * Cascade: pupilSubjectEnrollment → selected_subjects (name match).
 * Does NOT fall back to class.subjects — that can mark the wrong student complete
 * under concurrent enrollment / result-entry races.
 */
...
function sameIdSet(a, b) {
  const left = uniqueSubjectIds(a)
  const right = uniqueSubjectIds(b)
...
export async function checkAndNotifyParent({
  ...
  if (!planIncludes(String(school?.plan || '').toLowerCase(), 'sms-alerts', school)) {
    return
  }
  ...
  const prepared = await prismaClient.$transaction(async (tx) => {
    ...
    // Re-resolve under lock — reject if enrollment set changed mid-flight (concurrent edits).
    const enrolledUnderLock = await resolveEnrolledSubjectIds({
      schoolId,
      studentId,
      classId: effectiveClassId,
      student,
    ...
    if (!sameIdSet(enrolledSubjectIds, enrolledUnderLock)) {
      await tx.resultsStatus.update({
        where: { schoolId_studentId_term_year: key },
        data: {
          smsSending: false,
          smsLastAttemptAt: now,
          smsLastError: 'Enrollment changed during SMS evaluation',
        },
      })
      return { shouldSend: false }
    }

    const lockedResults = await tx.result.findMany({
      where: {
        schoolId,
        studentId,
        term,
        year,
        resultType: RESULT_TYPES.END_OF_TERM,
        subjectId: { in: enrolledUnderLock },
      },
      select: {
        subjectId: true,
        workflowStatus: true,
    ...
    const still = countFinalizedEnrolledSubjects(enrolledUnderLock, lockedResults)
    if (!still.isComplete) {
      await tx.resultsStatus.update({
        where: { schoolId_studentId_term_year: key },
        data: {
          smsSending: false,
          subjectsEnrolled: still.subjectsEnrolled,
          subjectsFinalized: still.subjectsFinalized,
          isComplete: false,
          completedAt: null,
          smsLastAttemptAt: now,
          smsLastError: 'Results incomplete after lock',
        },
      })
      return { shouldSend: false }
    }

    const gradeRows = (lockedResults || [])
      .filter(
        (r) =>
          String(r.workflowStatus || '')
            .trim()
    ...
    return {
      ...
      gradeRows,
      enrolledSubjectIds: enrolledUnderLock,
    }
  })
  ...
  const gradeRows = Array.isArray(prepared.gradeRows) ? prepared.gradeRows : []
