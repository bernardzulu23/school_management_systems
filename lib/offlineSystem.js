...
/**
 * Offline Data Storage using Dexie.js for robust IndexedDB management
 */
class OfflineDataStorage {
  constructor() {
    ...
    this.db.version(1).stores({
      offlineData: 'key, ++id, dataType, timestamp, priority',
    })
  }
  ...
  async store(key, data) {
    return await this.db.offlineData.put({
      key,
      ...data,
    })
  }
  ...
  async get(key) {
    return await this.db.offlineData.get(key)
  }

  async getByType(dataType) {
    return await this.db.offlineData.where('dataType').equals(dataType).toArray()
  }

  async getAll() {
    const allEntries = await this.db.offlineData.toArray()
    const result = {}
  ...
  async delete(key) {
    return await this.db.offlineData.delete(key)
  }
}
...
export class OfflineSystem {
  ...
  async handleOnlineMode() {
    ...
    const offlineDuration = this.offlineStartTime
      ? (new Date() - this.offlineStartTime) / (1000 * 60 * 60 * 24)
      : 0
  ...
  getApiEndpoint(dataType, action) {
    ...
    const endpoints = {
      ...
      timetable: `${baseUrl}/timetable/view`,
      communication: `${baseUrl}/sms/broadcast`,
      wellbeing: `${baseUrl}/student/goals`,
      gamification: `${baseUrl}/dashboard/student/games`,
    }

    return endpoints[dataType] || `${baseUrl}/ping`
  }
  ...
  async handleLowStorage() {
    console.log('Low storage detected - cleaning up old data')
  ...
  startOfflineMonitoring() {
    setInterval(() => {
      ...
      if (status.remainingOfflineDays < 1 && !this.isOnline) {
        console.warn('Approaching offline time limit')
  ...
  showLowOfflineTimeWarning() {
    ...
    warning.innerHTML = `
      <div style="
        ...
        z-index: 10000;
      ">
        <strong>Low Offline Time</strong><br>
  ...
  async loadOfflineData() {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('offline-data-loaded', {
