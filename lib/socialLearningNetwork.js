...
export class SocialLearningNetwork {
  ...
  static createStudyGroup(groupData) {
    const groupType =
      this.STUDY_GROUP_TYPES[groupData.type] || this.STUDY_GROUP_TYPES.SUBJECT_FOCUSED
  ...
  static createCollaborativeLearningSession(sessionData) {
    const activity =
      this.COLLABORATION_ACTIVITIES[sessionData.activityType] ||
      this.COLLABORATION_ACTIVITIES.STUDY_SESSION
  ...
  static matchLearningPartners(userId, preferences) {
    ...
    // Peer matching requires configured school profiles — not enabled by default.
    matching.potentialMatches = []
