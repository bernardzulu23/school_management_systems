...
export async function fetchResultsOverview({
  ...
  const normalizedResultType = resultType ? normalizeResultType(resultType) : ''
  if (isHeadteacher || isHod) {
    if (normalizedResultType && SCHOOL_WIDE_RESULT_TYPES.includes(normalizedResultType)) {
      andClauses.push({ resultType: normalizedResultType })
    } else {
      andClauses.push({ resultType: { in: SCHOOL_WIDE_RESULT_TYPES } })
    }
  } else if (isTeacherOnly) {
    if (normalizedResultType) {
      andClauses.push({ resultType: normalizedResultType })
    }
  } else if (normalizedResultType) {
    andClauses.push({ resultType: normalizedResultType })
  }
  ...
  const results = rows.map((r) => {
    ...
    const gradeInfo = calculateGrade(r.score, className)
    return {
      id: r.id,
      ...
      grade_status: gradeInfo.status,
      result_type: r.resultType || RESULT_TYPES.END_OF_TERM,
