...
import { setSchoolContext } from '@/lib/db/school-context'
...
export async function resolveAuthenticatedSchoolId(request, authUser) {
  ...
  // Prefer JWT schoolId as RLS GUC hint (not trusted until compared to DB).
  const jwtSchoolHint = authUser.schoolId ? String(authUser.schoolId).trim() : ''

  let dbUser
  try {
    dbUser = await prisma.$transaction(async (tx) => {
      if (jwtSchoolHint) {
        await setSchoolContext(tx, jwtSchoolHint)
      }
