 ...
 * }} [options]
 * @returns {{ subject: string, records: CurriculumRecord[], forms: number[], grades: number[], educationLevel: 'primary' | 'secondary' }}
 */
export function parseCdcSyllabusText(rawText, options = {}) {
  ...
  const educationLevel = options.educationLevel === 'primary' ? 'primary' : 'secondary'
  const syntax = educationLevel === 'primary' ? PRIMARY_SYNTAX : SECONDARY_SYNTAX
  ...
  function flush() {
    ...
    const id = `${educationLevel === 'primary' ? 'G' : 'F'}${formNum}-T${topicIdx}-S${subIdx}`
...
export function cdcRecordsToUnitCurriculum(parsed, meta = {}) {
  ...
  for (const r of parsed.records) {
    ...
    if (!byTopic.has(key)) {
      byTopic.set(key, {
        ...
        title: `${parsed.educationLevel === 'primary' ? 'Grade' : 'Form'} ${r.form}: ${r.topic}`,
        topics: [],
        learningOutcomes: [],
        suggestedActivities: [],
        assessmentMethods: [],
  ...
  const levels = parsed.educationLevel === 'primary' ? parsed.grades : parsed.forms
  ...
  return {
    ...
    level: `${label} ${minLevel}-${maxLevel}`,
    gradesCovered:
      parsed.educationLevel === 'primary' ? levels : levels.map((form) => Math.min(12, form + 7)),
    totalDuration: `${Math.max(1, maxLevel - minLevel + 1)} years (${label}s ${minLevel}-${maxLevel})`,
    units,
    metadata: {
      source:
        meta.source ||
