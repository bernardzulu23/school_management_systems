...
export function formatCurriculumRecord(record, meta = {}) {
  ...
  // Preserve the legacy Chemistry header when meta has no subject (Chemistry gold
  // corpus) so buildChemistryCurriculumContext regression stays byte-identical.
  const subjectLabel = String(meta.subject || '').trim()
  const isPrimary =
    record.grade != null ||
    String(meta.level || '')
      .toLowerCase()
      .includes('primary')
  const syllabusHeader = subjectLabel
    ? `Zambia CDC 2024 ${subjectLabel} Syllabus (${
        isPrimary ? 'Primary Education' : 'Ordinary Level'
      })`
    : 'Zambia CDC 2024 Chemistry Syllabus (Ordinary Level)'

  const lines = [
    syllabusHeader,
    `${record.grade != null ? `Grade ${record.grade}` : `Form ${record.form}`} | Topic ${
      record.topicNumber
    }: ${record.topic}`,
