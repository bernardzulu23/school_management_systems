...
export function decodeCdcLine(line, shift = DEFAULT_SHIFT) {
  const s = String(line || '')
  // Some PDF extractors split a custom-font row into fragments. The first
  // fragment can lose the encoded-space control byte (for example `&DUHHU`
  // still means `Career`). Decode those visibly encoded fragments too while
  // leaving ordinary mixed-case English lines untouched.
  ...
  if (/[\x01-\x1f]/.test(s)) return decodeFragment(s)

  // Mixed extraction can produce native text plus one encoded token, such as
  // `TOOLS AND (48,30(17`. Decode only the suspicious token so the native
  // prefix is not shifted.
  return s
    .split(/(\s+)/)
    .map((token) => {
      const suspicious =
      ...
      return suspicious ? decodeFragment(token) : token
    })
    .join('')
}
...
export function decodeCdcTextAuto(text, options = {}) {
  ...
  if (options.shift != null && Number.isFinite(options.shift)) {
    ...
    return {
      text: shift === 0 ? String(text || '') : decodeCdcText(String(text || ''), shift),
      shift,
      confidence: 1,
    }
  }
  const detected = decodeWithDetectedShift(String(text || ''), { sample: options.sample })
  return {
    ...detected,
    text:
      detected.shift === 0 ? String(text || '') : decodeCdcText(String(text || ''), detected.shift),
  }
}
