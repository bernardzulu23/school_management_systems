...
/**
 * Record of work — teaching weeks auto-filled from APPROVED lesson plans.
 * Mid/EOT test weeks are labeled as assessment-only (not “Not taught”).
 */
export async function exportRecordOfWorkTemplate(input: {
  ...
  const weekCount = Math.max(1, Math.min(20, Number(input.weekCount) || 12))
  ...
  const rows = [
    ...
    ...Array.from({ length: weekCount }, (_, i) => {
      ...
      const isTest = Boolean(data?.isTestWeek)
      const taught = Boolean(data?.taught && (data.topic || data.dateTaught || isTest))
      return new TableRow({
        children: [
          cell(String(week), { width: 8 }),
          cell(taught && !isTest ? data?.dateTaught || '' : '', { width: 14 }),
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({
            ...
            children: [
              new TextRun({
                text: 'Teaching weeks: auto-filled from APPROVED lesson plans only. Mid/EOT test weeks are assessment-only (not “Not taught”). Blank / “Not taught” = no approved plan for a teaching week.',
                italics: true,
                size: 16,
                color: '666666',
              }),
