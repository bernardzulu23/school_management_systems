...
function deriveSubjectFromFilename(filename) {
  let base = String(filename || '')
    ...
    .replace(/\s*\(\d+\)\s*$/g, '')
    .replace(/[_-]+/g, ' ')
    .replace(/\bsenior\s+preparation\b/gi, ' ')
    .replace(/\((?:stem\s+)?syllabi\)/gi, ' ')
    .replace(/\bsyllabi\b/gi, ' ')
  ...
  for (const line of lines) {
    ...
    if (outcomeMatch) {
      ...
      currentSubtopic.specificOutcomes.push({
        outcomeId,
        statement: statement.length >= 3 ? statement : `OCR fragment: ${statement || outcomeId}`,
        knowledge: [],
        skills: [],
        values: [],
      })
