...
import {
  ...
  resolveSchoolSectionFlags,
} from '@/lib/school/resolveSchoolSectionFlags'
...
export async function schoolAllowsSeniorTeacherFeatures(db, school) {
  if (!school?.id) return canUseSeniorTeacherFeatures(school?.level)
  if (normalizeLevel(school?.level) === 'secondary') return false
  if (normalizeLevel(school?.level) === 'primary') return true
  ...
  const flags = resolveSchoolSectionFlags(school, classes)
  return flags.hasPrimary === true
}

export { isPrimaryClassRecord }
...
export async function authorizeSeniorTeacherAdmin(request) {
  ...
  if (!(await schoolAllowsSeniorTeacherFeatures(prisma, school))) {
    return {
      ok: false,
