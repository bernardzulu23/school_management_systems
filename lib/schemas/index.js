 ...
 */
import { z } from 'zod'
import { ECZ_PRACTICE_EXAM_LEVELS, LEGACY_ECZ_EXAM_LEVELS } from '@/lib/ecz/ecz-practice-levels'
...
/**
 * POST /api/student/flashcards — generate an AI deck (1 per subject/day unless forceRefresh).
...
export const GenerateFlashcardsSchema = z
  .object({
    ...
    date: z
      ...
      .optional(),
    variationSeed: z.string().trim().min(1).max(80).optional(),
...
export const LEGACY_MOCK_EXAM_LEVELS = LEGACY_ECZ_EXAM_LEVELS.map((l) => l.value)
export const ALL_MOCK_EXAM_LEVELS = [...MOCK_EXAM_LEVELS, ...LEGACY_MOCK_EXAM_LEVELS]
...
export const NationalPercentileQuerySchema = z.object({
  subject: z.string().trim().min(1).max(120),
  examLevel: z.enum(ALL_MOCK_EXAM_LEVELS),
...
export const NotificationPreferencesSchema = z
  .object({
    ...
    expoPushEnabled: z.boolean().optional(),
  ...
  .refine(
    (data) => {
      if (
        ...
        data.expoPushEnabled === undefined &&
