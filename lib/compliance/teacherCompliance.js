...
export async function evaluateTeacherCompliance(schoolId) {
  ...
  const { start: dayStart, end: dayEnd } = todayBoundsLusaka()
  ...
  const [assessments, results, eczAssessments, eczScores, attendanceSessions] = await Promise.all([
  ...
  const attendanceByUser = new Set(attendanceSessions.map((s) => s.teacherId))
  ...
  for (const t of activeTeachers) {
    ...
    if (domains.attendance === 'missing') {
      ...
      flagged.push({
        ...
        teacherName: name,
        domain: 'attendance',
        reason: 'No attendance session opened today',
      })
    }
  }
