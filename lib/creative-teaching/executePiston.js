...
export function isPistonConfigured() {
  return Boolean(String(process.env.PISTON_API_KEY || process.env.PISTON_API_TOKEN || '').trim())
}
...
export async function executeViaPiston({ languageId, version, code }) {
  ...
  if (!res.ok) {
    const message =
      ...
      (res.status === 401
        ? 'Code execution service requires PISTON_API_KEY (contact your administrator).'
