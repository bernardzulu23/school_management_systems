...
/**
 * Drop pending mutation(s) for a channel + entityKey (e.g. cancel local goal create).
 */
export async function cancelPendingMutation(channel, entityKey) {
  const database = getOfflineDB()
  if (!database || !entityKey) return 0
  ...
  const existing = await database.mutationQueue
    .filter(
      (row) =>
        pendingFilter()(row) &&
        String(row.channel || '') === ch &&
        String(row.entityKey || '') === key
    )
    .toArray()
  for (const row of existing) {
    await database.mutationQueue.delete(row.id)
