...
class ApiClient {
  constructor() {
    ...
    this.client.interceptors.request.use(
      (config) => {
        ...
        // Sanitize plain objects only — never mutate FormData / Blob bodies.
        if (
          config.data &&
  ...
  async getHeadteacherDashboard(params = {}) {
    ...
    if (params?.resultType && String(params.resultType).trim()) {
      qs.set('resultType', String(params.resultType).trim())
    }
  ...
  async getResultsOverview(params = {}) {
    const qs = new URLSearchParams()
    ...
    if (params?.resultType) qs.set('resultType', String(params.resultType))
    if (params?.limit) qs.set('limit', String(params.limit))
    const suffix = qs.toString()
    return this.get(`/dashboard/results${suffix ? `?${suffix}` : ''}`)
  }
  ...
  async toggleMaterialBookmark(materialId, bookmarked) {
    const body = { materialId, action: 'bookmark' }
    if (typeof bookmarked === 'boolean') body.bookmarked = bookmarked
    return this.post('/student/materials', body)
  }
  ...
  async assignStudentBusRoute(data) {
    return this.post('/transport/assignments', data)
  }

  async getHostelRooms(params) {
    return this.get('/hostel/rooms', params)
  }

  async createHostelRoom(data) {
    return this.post('/hostel/rooms', data)
  }

  async assignStudentHostel(data) {
    return this.post('/hostel/assignments', data)
  }

  async getHouses(params) {
    return this.get('/houses', params)
  }
  ...
  async assignStudentHouse(data) {
    return this.post('/houses/assignments', data)
  }

  async removeStudentHouse(params) {
    return this.delete('/houses/assignments', { params })
  }

  async getFeeSchedules(params) {
    return this.get('/fees/schedules', params)
  }

  async createFeeSchedule(data) {
    return this.post('/fees/schedules', data)
  }

  async getFeeInvoices(params) {
    return this.get('/fees/invoices', params)
  }

  async generateFeeInvoices(data) {
    return this.post('/fees/invoices/generate', data)
  }
  ...
  async getFeeSummary() {
    return this.get('/fees/summary')
  }

  async getSiblingGroups() {
    return this.get('/fees/siblings')
  }

  async createSiblingGroup(data) {
    return this.post('/fees/siblings', data)
  }

  async getParentPortal() {
    return this.get('/parent/portal')
  }

  async getProprietorOverview() {
    return this.get('/proprietor/overview')
  }
  ...
  async likeStudentWork(id) {
    return this.post(`/student-works/${id}/like`)
  }
  ...
  async getAssessmentsTeacherOverview() {
    return this.get('/assessments/teacher-overview')
  }
  ...
  // File upload — do not force Content-Type (browser must set multipart boundary).
  async uploadFile(file, { purpose = 'study' } = {}) {
  ...
  async getTimetable(id) {
    return this.get('/timetable/view', { id })
  }

  async createTimetable(data) {
    return this.post('/timetable/generate', data)
  }

  async updateTimetable(id, data) {
    return this.put('/timetable/entries', { ...data, id })
  }

  async deleteTimetable(id) {
    return this.delete('/timetable/entries', { data: { id } })
  }

  async getTimetableDraftMeta({ term, academicYear, refresh = false } = {}) {
    const params = { term, academicYear }
    if (refresh) params.refresh = 'true'
    return this.get('/timetable/draft-meta', params)
  }

  async scanTimetableConflicts({ term, academicYear } = {}) {
    return this.get('/timetable/conflicts', { term, academicYear })
  }

  async dismissTimetableDraftAudit({ term, academicYear, auditKeys, auditKey, mode = 'add' } = {}) {
    return this.patch('/timetable/draft-meta', {
      term,
      academicYear,
      ...(auditKeys ? { auditKeys } : auditKey ? { auditKey } : {}),
      mode,
    })
  }

  async clonePublishedTimetableToDraft({ term, academicYear } = {}) {
    return this.post('/timetable/entries/clone-published-to-draft', { term, academicYear })
  }

  async copyTimetableFromPreviousTerm({
    sourceTerm,
    sourceAcademicYear,
    targetTerm,
    targetAcademicYear,
    ...
    return this.post('/timetable/entries/copy-from-term', {
      sourceTerm,
      sourceAcademicYear,
      targetTerm: targetTerm || term,
      targetAcademicYear: targetAcademicYear || academicYear,
      sourceStatus,
    })
  }

  async resolveTimetableConflict(payload) {
    return this.post('/timetable/conflicts/resolve', payload)
  }
