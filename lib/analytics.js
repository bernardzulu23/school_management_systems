...
export class AssignmentTracker {
  static analyzeAssignmentCompletion(assignments) {
    ...
    const totalRelevant = relevantAssignments.length

    if (totalRelevant === 0) {
      return analysis
    }

    const completed = relevantAssignments.filter(a => a.status === 'completed').length
    const late = relevantAssignments.filter(a => a.status === 'late').length
    const missed = relevantAssignments.filter(a => a.status === 'missing').length

    analysis.completionRate = Math.round((completed / totalRelevant) * 100)
