...
export class AES256Encryption {
   ...
   * Server: SERVER_ENCRYPTION_KEY only (never NEXT_PUBLIC_* — those ship to the browser).
   * Browser: per-device key in localStorage (generated client-side).
   */
  initializeKey() {
    if (typeof window === 'undefined') {
      const serverKey = process.env.SERVER_ENCRYPTION_KEY
      if (!serverKey && process.env.NODE_ENV === 'production' && !process.env.NEXT_PHASE) {
        console.warn('SERVER_ENCRYPTION_KEY missing in production environment')
      }
      return serverKey || 'dev-only-fallback-key-replace-in-prod'
    }
    ...
    try {
      ...
      console.warn('LocalStorage access failed; generating ephemeral device key')
  ...
  validateSecureToken(token) {
    try {
      const encryptedData = JSON.parse(
        CryptoJS.enc.Utf8.stringify(CryptoJS.enc.Base64.parse(token))
      )
