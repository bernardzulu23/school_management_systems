...
export async function publishAssessmentAsAssignment({
  ...
  const aiAnalysis =
    assessment.aiAnalysis && typeof assessment.aiAnalysis === 'object' ? assessment.aiAnalysis : {}
  const sourceMaterials = Array.isArray(aiAnalysis.sourceMaterialTitles)
    ? aiAnalysis.sourceMaterialTitles
    : []
  ...
  const payload = buildInteractiveQuizPayload(quiz, {
    ...
    sourceMaterials,
    materialIds: Array.isArray(aiAnalysis.materialIds) ? aiAnalysis.materialIds : [],
  })
