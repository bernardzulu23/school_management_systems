    ...
    [this.SKILL_CATEGORIES.MATHEMATICS]: {
      ...
      skills: {
        ...
        calculus_master: {
          ...
          rewards: {
            points: 500,
            badge: 'calculus_legend',
            nft: 'mathematical_crown',
            title: 'Math Virtuoso',
          },
        },
      },
...
export class NFTAchievementSystem {
  ...
  static generateUniqueNFTId(studentId, nftId) {
    return `${nftId}-${studentId}-${Date.now().toString(36)}`
  }
 ...
 * Central hub for managing all gamification aspects
 */
export class GamificationManager {
  processStudentData(studentData) {
    if (!studentData) return null

    // Use API-provided data if available
    const points = studentData.gamificationProfile?.points || studentData.points || 0
    const level = studentData.gamificationProfile?.level || studentData.level || 1
    const xp = studentData.gamificationProfile?.xp || studentData.xp || 0
    const nextLevelXp =
      studentData.gamificationProfile?.nextLevelXp || studentData.nextLevelXp || 100

    // Evaluate achievements
    const achievementResults = AchievementSystem.evaluateAchievements(studentData)

    // Get skill tree progress
    const skillTreeData = studentData.skills || {}
    const skillProgress = SkillTreeSystem.getStudentSkillProgress(studentData.id, skillTreeData)

    // Generate NFT collection (mock)
    const nftCollection = [] // Implement NFT fetching logic
    ...
    return {
      ...
      achievements: achievementResults.earnedAchievements,
      achievementProgress: achievementResults.progress,
      skillTrees: skillProgress,
      nftCollection: nftCollection,
      leaderboardRank: studentData.rank || 0,
    }
  }
}
