...
/**
 * SBA submission deadline for an academic year (31 January of the following year).
 * @param {number|string} academicYear
 * @returns {Date}
 */
export function getSBASubmissionDeadline(academicYear) {
  const year = Number(academicYear)
  return new Date(year + 1, SBA_SUBMISSION_DEADLINE_MONTH, SBA_SUBMISSION_DEADLINE_DAY, 23, 59, 59)
}

/**
 * ECZ SBA weight by subject — PE is 40%, all other subjects 30%.
...
export function getSBAWeight(subjectName) {
  ...
  if (PE_SUBJECTS.includes(name) || name.toUpperCase() === 'PE') return 40
  return SBA_WEIGHT_PERCENT
}
