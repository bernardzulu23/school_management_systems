...
export function SchoolProvider({ children }) {
  ...
  const fetchSchool = useCallback(async ({ allowCache = true } = {}) => {
    const subdomain = normalizeSchoolSubdomain(resolveSubdomain()) || ''
    // v6: apex/www are platform hosts (never cached as school "www")
    const cacheKey = subdomain ? `school-cache:v6:${subdomain}` : 'school-cache:v6:default'
