...
export function HeadteacherProvider({ children }) {
  ...
  const { data: stats, isLoading: statsLoading, error: statsError } = useQuery({
    ...
    queryFn: () => api.getDashboardStats().then(res => res.data).catch(err => {
  ...
  const { data: dashboardData, isLoading: dashboardLoading, error: dashboardError } = useQuery({
    ...
    queryFn: () => api.getHeadteacherDashboard().then(res => res.data).catch(err => {
  ...
  const hasResults = useMemo(() => 
    dashboardData?.subject_performance && Object.keys(dashboardData.subject_performance).length > 0,
  [dashboardData]);
  ...
  return (
    <HeadteacherContext.Provider value={value}>
      {children}
    </HeadteacherContext.Provider>
  );
}
