...
export function HeadteacherProvider({ children }) {
  ...
  const {
    ...
    queryKey: ['headteacher-dashboard', selectedTerm, selectedResultType],
    queryFn: () =>
      api
  ...
  const schoolStats = useMemo(() => {
    ...
    return {
      ...
      studentAchievement: data
        ? (data.studentAchievement ?? data.student_achievement ?? unknown)
        : unknown,
      passRate: data ? (data.passRate ?? data.pass_rate ?? unknown) : unknown,
      teacherEffectiveness: data
        ? (data.teacherEffectiveness ?? data.teacher_effectiveness ?? unknown)
        : unknown,
      complianceRate: data ? (data.complianceRate ?? data.compliance_rate ?? unknown) : unknown,
      teacherDevelopment: data
        ? (data.teacherDevelopment ?? data.teacher_development ?? unknown)
        : unknown,
    }
  }, [stats, dashboardData])
