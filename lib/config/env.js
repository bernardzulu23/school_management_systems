...
const OPTIONAL = {
  ...
  AFRICASTALKING_API_KEY: "Africa's Talking API key — cloud SMS (after Android gateway)",
  AFRICASTALKING_USERNAME: "Africa's Talking username",
  AFRICAS_TALKING_API_KEY: 'Alias for AFRICASTALKING_API_KEY',
  AFRICAS_TALKING_USERNAME: 'Alias for AFRICASTALKING_USERNAME',
  ZSMS_ONBOARDING_SENDER_ID: 'Onboarding welcome SMS sender (default ZSMS)',
  SMS_GATEWAY_ENCRYPTION_KEY:
    'AES-256 key for Android gateway pairing tokens (openssl rand -hex 32)',
  ...
  VERCEL_API_TOKEN: 'Vercel API token — read Web Analytics for /platform/overview',
  VERCEL_PROJECT_ID: 'Vercel project id (defaults to production ZSMS project)',
  VERCEL_TEAM_ID: 'Vercel team id for Analytics API queries',
  ...
  OPENROUTER_API_KEY: 'OpenRouter AI key — chat fallback + optional RAG embeddings (openrouter.ai)',
  VAPID_PUBLIC_KEY: 'Web push VAPID public key (browser notifications)',
  VAPID_PRIVATE_KEY: 'Web push VAPID private key',
  VAPID_SUBJECT: 'Web push contact mailto: URL',
  AWS_REGION: 'AWS region for SNS critical SMS alerts',
  AWS_ACCESS_KEY_ID: 'AWS IAM access key for SNS',
  AWS_SECRET_ACCESS_KEY: 'AWS secret for SNS',
  AWS_SNS_TOPIC_ARN: 'Optional SNS topic for notification SMS',
  AWS_SNS_SENDER_ID: 'SMS sender ID for SNS (default ZSMS)',
}
...
export function validateEnv() {
  ...
  if (!hasGroq && !hasGemini && !hasOpenRouter) {
    missing.push(
      '  ❌ GROQ_API_KEY, GEMINI_API_KEY, or OPENROUTER_API_KEY — at least one AI provider key is required for AI features'
    )
  }
  ...
  if (missing.length > 0) {
    ...
    // Dependabot / Preview PR builds often lack Production secrets. Let the
    // compile finish; runtime + production builds still fail closed.
    const softBuild =
      isNextBuild &&
    ...
    if (softBuild) {
      console.warn(
        `${message}\n(Continuing non-production build without secrets; runtime will still require them.)\n`
      )
      return
    }
...
export const env = {
  ...
  // AI (Groq primary; Gemini → OpenRouter → OpenAI → HuggingFace fallbacks)
  ...
  // SMS — read at access time so Vercel env is never snapshotted as empty at import
  get atApiKey() {
    return process.env.AFRICASTALKING_API_KEY || process.env.AFRICAS_TALKING_API_KEY
  },
  get atUsername() {
    return process.env.AFRICASTALKING_USERNAME || process.env.AFRICAS_TALKING_USERNAME
  },
