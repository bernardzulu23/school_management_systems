...
export const ERROR_MESSAGES = {
  ...
  /** Permission denied — do not imply the session is invalid. */
...
export function toUserFacingMessage(input, fallback = ERROR_MESSAGES.GENERIC) {
  ...
  if (
    ...
    (lower.includes('please log in again') && !lower.includes('not authorized'))
  ) {
    return ERROR_MESSAGES.SESSION_EXPIRED
  }
  if (lower.includes('csrf')) return ERROR_MESSAGES.CSRF
  // Bare Forbidden / unauthorized = permission denial (not a forced re-login).
  if (lower === 'forbidden' || lower === 'unauthorized' || lower.includes('not authorized')) {
...
export function userFacingFromHttp(status, body, fallback = ERROR_MESSAGES.GENERIC) {
  ...
  if (status === 401) {
    ...
    return toUserFacingMessage(
      body?.message || body?.error || ERROR_MESSAGES.SESSION_EXPIRED,
  ...
  if (status === 403) {
    ...
    return toUserFacingMessage(
      body?.message || body?.error || ERROR_MESSAGES.UNAUTHORIZED,
  ...
  if (status === 503 || code === 'AI_UNAVAILABLE') {
    return toUserFacingMessage(body?.message || body?.error, ERROR_MESSAGES.AI_UNAVAILABLE)
  }
  if (status >= 500) {
