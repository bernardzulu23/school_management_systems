 ...
 */
/**
 * Poll Lipila check-status with backoff: 5s, 15s, 30s, 60s (max ~3 min).
 * @returns {Promise<'paid'|'failed'|'pending'|'timeout'>}
 */
export async function checkPaymentStatusWithRetry(referenceId, maxWaitMs = 180_000) {
  ...
  while (Date.now() - started < maxWaitMs) {
    ...
    const result = await lipilaCheckCollectionStatus(referenceId, {
    ...
    if (result.ok && result.data) {
      ...
      if (isPaidLipilaStatus(status)) return 'paid'
      if (isFailedLipilaStatus(status)) return 'failed'
    }

    const wait = delays[Math.min(attempt, delays.length - 1)]
    attempt += 1
    if (Date.now() - started + wait >= maxWaitMs) break
