...
export function useAsync() {
  ...
  const execute = useCallback(async (asyncFunction, successMessage = null, retryOptions = { maxRetries: 0, delay: 1000 }) => {
    let attempt = 0;
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isNetworkError && attempt < maxRetries) {
          attempt++;
          const delay = retryOptions.delay * Math.pow(2, attempt - 1); // Exponential backoff
          toast.loading(`Network issue. Retrying in ${delay/1000}s... (Attempt ${attempt}/${maxRetries})`, { id: 'retry-toast' });
          await new Promise(resolve => setTimeout(resolve, delay));
          toast.dismiss('retry-toast');
          return run();
        }

        const message = err.response?.data?.error || err.message || ERROR_MESSAGES.SERVER_ERROR;
