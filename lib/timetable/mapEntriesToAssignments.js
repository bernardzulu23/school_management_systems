...
export function mapDbEntriesToAssignments(entries) {
  return (entries || []).map((e) => {
    ...
    return {
      ...
      endTime: e.endTime,
      period: Number(e.periodNumber) >= 1 ? Number(e.periodNumber) : 1,
      teacherId: e.teacherId,
      teacherName:
        e.teacherName ||
        (String(e.teacherId) === String(e.allocation?.teacher?.id)
          ? e.allocation?.teacher?.name
          : '') ||
        e.allocation?.teacher?.name ||
        '',
      classId: e.classId,
      className: e.allocation?.class?.name || e.className || '',
      classroomId: e.classroomId || e.allocation?.classroomId || undefined,
      classroomName: e.classroom?.name || e.classroomName || e.allocation?.classroom?.name || '',
      subjectId: e.subjectId,
      subjectName: e.allocation?.subject?.name || e.subjectName || '',
      durationMin: doubleFields.durationMin,
