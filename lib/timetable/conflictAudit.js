...
function mapValidationToUiConflict(c, assignmentById, index) {
  ...
  if (c.type === 'TEACHER_DOUBLE_BOOKED' && a0) {
    ...
    const subjects = [...new Set(members.map((a) => a?.subjectName).filter(Boolean))]
    const hasTimeOverlap = members.some((a, i) =>
      members.slice(i + 1).some((b) => timesOverlap(a, b))
    )
    const when = a0.dayOfWeek && a0.startTime ? ` (${a0.dayOfWeek} ${a0.startTime})` : ''
    const className = a0.className || 'Class'
    if (subjects.length <= 1 && ids.length > 2 && hasTimeOverlap) {
      description = `${className} has ${ids.length} duplicate ${subjects[0] || 'lesson'} entries at the same slot${when}`
    } else if (subjects.length <= 1 && !hasTimeOverlap) {
      description = `${className} has ${subjects[0] || 'the same subject'} more than once on ${a0.dayOfWeek || 'the same day'}${when}`
    } else {
      description = `${className} is scheduled for ${subjects.join(' and ') || 'multiple subjects'} at the same time${when}`
    }
