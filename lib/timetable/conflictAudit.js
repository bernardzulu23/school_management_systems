...
export async function auditDraftTimetable(prisma, { schoolId, term, academicYear }) {
  ...
  let conflicts = [...validationConflicts, ...workloadConflicts, ...missingConflicts]
  conflicts = filterConflictsForActiveClasses(conflicts, assignments)
  conflicts = enrichConflictsWithSuggestions(conflicts, assignments)
...
export function formatDraftMetaResponse(meta, { term, academicYear, includeSummary = true } = {}) {
  ...
  return {
    ...
    ...(includeSummary ? { conflictSummary: meta.conflictSummary ?? [] } : {}),
  }
}
