...
export const ROOM_OVERLAP_CONSTRAINT = 'TimetableAllocationEntry_no_room_overlap'
...
export function interpretTimetableExcludeError(err) {
  ...
  const mentionsRoom =
    msg.includes(ROOM_OVERLAP_CONSTRAINT) || metaConstraint.includes(ROOM_OVERLAP_CONSTRAINT)
  const isExclusion =
    code === '23P01' ||
    mentionsClass ||
    mentionsTeacher ||
  ...
  if (mentionsRoom && !mentionsClass && !mentionsTeacher) {
    return {
      isExcludeViolation: true,
      code: 'ROOM_DOUBLE_BOOKED',
      constraint: ROOM_OVERLAP_CONSTRAINT,
      message:
        "Can't place this lesson here — that room/lab is already booked by another class at this time.",
    }
  }
 ...
 * @param {{ teacherName?: string, className?: string, roomName?: string, dayOfWeek?: string, startTime?: string, endTime?: string }} ctx
 */
export function formatTimetableExcludeMessage(base, ctx = {}) {
  ...
  const when = [
    ...
    .filter(Boolean)
    .join(' ')
  if (base.code === 'ROOM_DOUBLE_BOOKED' && ctx.roomName) {
    return `Can't move this lesson here — ${ctx.roomName} is already booked at this time${when ? ` (${when})` : ''}.`
  }
