...
export function rowSpanForAssignment(assignment, bellRows) {
  ...
  // Lazy import avoided — duplicate minimal logic via effective span inline
  const cp = Math.max(1, Number(assignment.consecutivePeriods) || 1)
  const isDouble =
    assignment.isDoublePeriod === true ||
    cp >= 2 ||
    String(assignment.periodType || '')
      .toLowerCase()
      .includes('double')
  ...
  let span = isDouble ? Math.max(2, cp) : cp
  if (startIdx >= 0 && bellRows[startIdx]?.isDouble) span = Math.max(span, 2)
