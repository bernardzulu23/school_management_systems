...
export async function syncDepartmentApprovalToTeacherAllocations(tx, params) {
  const { schoolId, departmentAllocationId, allocationData, hodUserId, term, academicYear } = params

  const details = unwrapDepartmentAllocationData(allocationData)
  if (!details.teacherId) throw new Error('Allocation missing teacherId')
