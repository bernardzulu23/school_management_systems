...
/** Local/dev only — never enable TLS bypass on Vercel or production. */
function shouldBypassTlsVerify() {
  if (process.env.VERCEL || process.env.NODE_ENV === 'production') return false
  // Explicit opt-out for local if needed
  const flag = String(process.env.SMS_AT_INSECURE_TLS || '')
    .trim()
    .toLowerCase()
...
function readAtCredentials() {
  // Prefer AFRICAS_TALKING_* (current local/Vercel naming); fall back to AFRICASTALKING_*.
  const apiKey = String(
    process.env.AFRICAS_TALKING_API_KEY || process.env.AFRICASTALKING_API_KEY || ''
  ).trim()
  const username = String(
    process.env.AFRICAS_TALKING_USERNAME || process.env.AFRICASTALKING_USERNAME || ''
  ).trim()
  return { apiKey, username, configured: Boolean(apiKey && username) }
}

function messagingUrl(username) {
  const isSandbox =
    String(username || '')
      .trim()
      .toLowerCase() === 'sandbox'
...
/**
 * POST to Africa's Talking messaging API.
 * Uses insecure TLS agent only for local testing (never on Vercel/production).
 */
async function postMessaging(payload) {
  const { apiKey, username } = readAtCredentials()
  const body = {
    username,
  ...
  if (payload.enqueue === false) body.enqueue = 0
  else if (payload.enqueue !== undefined) body.enqueue = 1

  const bypassTls = shouldBypassTlsVerify()
  const axiosConfig = {
    headers: {
      apiKey,
  ...
  const response = await axios.post(messagingUrl(username), new URLSearchParams(body), axiosConfig)
  return response.data
}

async function sendWithClient(_client, payload) {
  // Prefer axios so we can attach a local-only TLS bypass agent.
  // Fall back to the SDK if axios path fails unexpectedly in production.
  try {
    const result = await postMessaging(payload)
    ...
    if (shouldBypassTlsVerify() || !_client) throw axiosErr
    const result = await _client.send(payload)
    const rows = result?.SMSMessageData?.Recipients || []
    const { accepted, rejected } = recipientOutcome(rows)
    return { result, rows, accepted, rejected }
  }
...
export async function sendSMS(phoneNumbers, message, from, options = {}) {
  ...
  try {
    ...
    if (!client && !creds.configured) {
      return { success: false, reason: 'SMS not configured', results: [] }
    }

    if (shouldBypassTlsVerify()) {
      log.warn("Africa's Talking TLS verify disabled (local testing only)")
    }
