...
export async function sendSMS(phoneNumbers, message, from, options = {}) {
  ...
  try {
    ...
    // AT often returns HTTP 200 with per-recipient failures (InvalidSenderId, etc.).
    const okCodes = new Set([100, 101, 102])
    const accepted = rows.filter((r) => okCodes.has(Number(r?.statusCode)))
    const rejected = rows.filter((r) => !okCodes.has(Number(r?.statusCode)))

    if (rows.length > 0 && accepted.length === 0) {
      const reason =
        rejected.map((r) => `${r?.status || 'Rejected'}(${r?.statusCode})`).join(', ') ||
        result?.SMSMessageData?.Message ||
        'All recipients rejected'
      log.warn("SMS rejected by Africa's Talking", { reason, recipients: rows.length })
      return { success: false, reason, results: rows }
    }

    log.info('SMS sent', {
      recipients: accepted.length || rows.length,
