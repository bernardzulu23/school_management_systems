...
/**
 * QStash Deduplication-Id must be URL-safe and cannot contain `:`.
 * Keep DB idempotency keys as-is; only sanitize what we send to QStash.
 */
export function toQstashDedupId(value, { maxLen = 128 } = {}) {
  const raw = String(value || '')
    .trim()
    .replace(/[^a-zA-Z0-9_-]+/g, '-')
    .replace(/-+/g, '-')
    .replace(/^-|-$/g, '')
  if (!raw) return `dedup-${Date.now()}`
  return raw.slice(0, Math.max(8, Number(maxLen) || 128))
}
