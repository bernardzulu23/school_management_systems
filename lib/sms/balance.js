...
/** Free SMS pack granted once when a school starts a trial. */
export const TRIAL_SMS_CREDITS = 50
...
/**
 * Add prepaid SMS credits. When `{ trial: true }`, applies at most once per school
 * (guarded by `trialSmsGrantedAt`).
 *
 * @param {string} schoolId
 * @param {number} amount
 * @param {{ trial?: boolean, tx?: import('@prisma/client').Prisma.TransactionClient }} [opts]
 */
export async function grantSmsCredits(schoolId, amount, opts = {}) {
  const n = Math.max(0, Math.floor(Number(amount) || 0))
  if (n === 0) {
    return { ok: false, reason: 'invalid_amount', granted: false }
  }

  const asTrial = Boolean(opts.trial)

  const run = async (tx) => {
    const existing = await tx.schoolSmsSettings.findUnique({ where: { schoolId } })

    if (asTrial && existing?.trialSmsGrantedAt) {
      return {
        ok: true,
        granted: false,
        reason: 'already_granted',
        smsBalance: existing.smsBalance,
        smsLifetimeGranted: existing.smsLifetimeGranted,
        smsLifetimeUsed: existing.smsLifetimeUsed,
        trialSmsGrantedAt: existing.trialSmsGrantedAt,
      }
    }

    const now = new Date()
    const row = await tx.schoolSmsSettings.upsert({
      where: { schoolId },
      create: {
        schoolId,
        smsBalance: n,
        smsLifetimeGranted: n,
        smsLifetimeUsed: 0,
        ...(asTrial ? { trialSmsGrantedAt: now } : {}),
      },
      update: {
        smsBalance: { increment: n },
        smsLifetimeGranted: { increment: n },
        ...(asTrial && !existing?.trialSmsGrantedAt ? { trialSmsGrantedAt: now } : {}),
      },
    })
    ...
    return {
      ...
      smsLifetimeGranted: row.smsLifetimeGranted,
      smsLifetimeUsed: row.smsLifetimeUsed,
      trialSmsGrantedAt: row.trialSmsGrantedAt,
    }
  }
...
export async function reserveSmsCredits(schoolId, count) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    const next = await tx.schoolSmsSettings.update({
      where: { schoolId },
      data: {
        smsBalance: { decrement: n },
        smsLifetimeUsed: { increment: n },
      },
    })
...
export async function refundSmsCredit(schoolId, count = 1) {
  ...
  await prisma.$transaction(async (tx) => {
    await tx.schoolSmsSettings.upsert({
      where: { schoolId },
      create: { schoolId, smsBalance: n, smsLifetimeGranted: n, smsLifetimeUsed: 0 },
    ...
    const nextUsed = Math.max(0, (row.smsLifetimeUsed ?? 0) - n)
    await tx.schoolSmsSettings.update({
      where: { schoolId },
      data: { smsLifetimeUsed: nextUsed },
    })
  })
}
