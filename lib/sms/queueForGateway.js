/**
 * Fork point: queue SMS for the platform shared Android SIM gateway
 * (or legacy per-school gateway during transition).
...
export async function queueForGatewayIfEnabled({ schoolId, to, message }) {
  ...
  try {
    ...
    const gateway = await resolveActiveGatewayForSchool(sid)
