 ...
 *
 * Mocean is retired — never dual-send AT + Mocean. On success, return immediately
 * without attempting the other channel (except AT failure → gateway fallback).
 *
...
export async function sendOutboundSms({
  ...
  // Hard stop if a legacy Mocean env is still present with a dual-send flag.
  const dual =
    String(process.env.SMS_DUAL_SEND_MOCEAN || process.env.ENABLE_MOCEAN || '')
      .trim()
      .toLowerCase() === 'true'
  if (dual) {
    console.error(
      '[sms] Refusing send: SMS_DUAL_SEND_MOCEAN/ENABLE_MOCEAN is set — Mocean cutover complete'
    )
    return {
      ok: false,
      recipients: normalizePhoneNumbers(to),
      provider: null,
      channel: null,
      reason: 'Mocean dual-send is disabled',
      response: null,
      queuedForGateway: false,
    }
  }
  ...
  try {
    const { recordSmsFailureAlert } = await import('@/lib/observability/alerts')
    recordSmsFailureAlert({
      schoolId,
      reason: atResult.reason || 'sms_send_failed',
      provider: atResult.provider || null,
    })
  } catch {
    // never block SMS path on observability
  }
