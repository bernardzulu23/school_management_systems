...
import { notifyParentsBatch } from '@/lib/attendance/parentNotifications'
...
export async function openAttendanceSession({
  ...
  let resolvedMethod = verificationMethod || 'MANUAL'
  if (resolvedMethod === 'FACE') {
    const { getSchoolFacialPolicy } = await import('@/lib/consent/facialAttendance')
...
async function assertTwinAllowed(
  sessionId,
  student,
  { schoolId, twinAuthToken, secondaryVerified }
) {
  ...
  if (!twinPresent || !student.requiresSecondaryAuth) return true

  if (twinAuthToken) {
    const { verifyTwinAuthTicket } = await import('@/lib/attendance/twinAuthTicket')
    const check = verifyTwinAuthTicket(twinAuthToken, {
      schoolId,
      sessionId,
      studentId: student.id,
    })
    if (check.ok) return true
    const err = new Error(check.message || 'Twin PIN verification required')
    err.code = check.code || 'TWIN_SECONDARY_AUTH_REQUIRED'
    err.status = 409
    throw err
  }

  // Client-asserted secondaryVerified is never accepted (Prompt 23).
  if (secondaryVerified) {
    const err = new Error(
      'Client-asserted secondaryVerified is not accepted. Complete twin PIN verification first.'
    )
    err.code = 'TWIN_AUTH_TOKEN_REQUIRED'
    err.status = 409
    throw err
  }
...
export async function recordAttendanceMark({
  ...
  const methodNorm = String(method || 'MANUAL').toUpperCase()
  if (methodNorm === 'FACE') {
    const { assertMayUseFaceMethod } = await import('@/lib/consent/facialAttendance')
    await assertMayUseFaceMethod(schoolId, studentId)
  }

  await assertTwinAllowed(sessionId, student, {
    schoolId,
    twinAuthToken,
    secondaryVerified,
  })
  ...
  if (
    notifyParents !== false &&
    ['PRESENT', 'LATE', 'ABSENT', 'EXCUSED'].includes(normalizedStatus)
  ) {
    // Import lazily to avoid circular init; Vercel-safe after-response send.
    const { scheduleParentAttendanceSms } = await import('@/lib/attendance/parentNotifications')
    scheduleParentAttendanceSms({
      studentId: student.id,
...
export async function sendChronicAbsenteeAlerts({
  ...
  if (notifyParents && smsAllowed) {
    for (const row of chronic) {
      ...
      await sendSchoolSms({ to: recipients, message, schoolId }).catch(() => {})
      pushSmsLog({
        direction: 'out',
        schoolId,
        to: recipients,
