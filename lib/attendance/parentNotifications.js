...
import { pushSmsLog, sendSchoolSms, getSchoolSmsFrom } from '@/lib/sms'
import { getOrCreateSmsSettings } from '@/lib/sms/balance'
import { runAfterResponse } from '@/lib/attendance/runAfterResponse'
...
async function loadAttendanceSmsContext(schoolId) {
  const [school, smsSettings] = await Promise.all([
    prisma.school.findUnique({
      where: { id: schoolId },
      select: { name: true, plan: true, trialEndsAt: true, planExpiresAt: true, active: true },
    }),
  ...
  const plan = String(school?.plan || '')
    .trim()
    .toLowerCase()
  // Pricing: SMS on Trial/Basic+ (inherited by Standard/Premium).
  const smsAllowed = planIncludes(plan, 'sms-alerts', school)
...
export async function notifyParentAttendance({
  ...
  try {
    ...
    const rawRecipients = normalizePhoneNumbers(extractParentContacts(student))
    if (rawRecipients.length === 0) {
      return { sent: false, reason: 'No parent phone number on record' }
    }

    const { filterPhonesNotOptedOut } = await import('@/lib/privacy/smsOptOut')
    const recipients = await filterPhonesNotOptedOut(schoolId, rawRecipients, 'attendance')
    if (recipients.length === 0) {
      return { sent: false, reason: 'Parent SMS opted out' }
    }
    ...
    if (!sendResult?.ok) {
      const reason = sendResult?.reason || sendResult?.failureReason || 'SMS send failed'
      console.warn('[ParentSMS] Provider did not send', {
        studentId,
        schoolId,
        status: normalizedStatus,
        reason,
      ...
      return { sent: false, reason, provider: sendResult?.provider || null }
    }
...
export async function notifyParentsBatch(marks, schoolId, sessionId, date, opts = {}) {
  ...
  const summary = { sent: 0, failed: 0, skipped: list.length, failures: [] }

  const ctx = await loadAttendanceSmsContext(schoolId)
  if (!ctx.smsAllowed) {
    console.warn('[BatchAttendanceSMS] SMS blocked by school plan', { schoolId })
    summary.skipped = list.length
    summary.reason = 'SMS not included in school plan'
    return summary
  }
  ...
  if (alertMarks.length === 0) {
    console.warn('[BatchAttendanceSMS] All marks skipped (toggles off or invalid status)', {
      schoolId,
      total: list.length,
      statuses: list.map((m) => mapMarkStatus(m?.status)),
      prefs: ctx.prefs,
    })
    return summary
  }
  ...
  for (let i = 0; i < alertMarks.length; i += BATCH_SIZE) {
    ...
    for (const r of results) {
      if (r.status === 'fulfilled' && r.value?.sent) {
        ...
        const reason =
          r.status === 'fulfilled'
            ? r.value?.reason || 'SMS send failed'
            : r.reason?.message || 'SMS send failed'
        if (summary.failures.length < 3 && !summary.failures.includes(reason)) {
...
/**
 * Schedule parent SMS after the HTTP response (Vercel-safe).
 * Plain fire-and-forget is killed when the serverless function freezes.
 */
export function scheduleParentAttendanceSmsBatch(params) {
  const marks = Array.isArray(params?.marks) ? params.marks : []
  if (!params?.schoolId || marks.length === 0) {
    console.log('[BatchAttendanceSMS] Nothing to send', {
  ...
  console.log('[BatchAttendanceSMS] Scheduling after response', {
    schoolId: params.schoolId,
    markCount: marks.length,
    statuses: marks.map((m) => m?.status).filter(Boolean),
  })

  runAfterResponse(async () => {
    const summary = await notifyParentsBatch(
      marks,
      params.schoolId,
      params.sessionId,
      params.date,
    ...
    console.log('[BatchAttendanceSMS] Done', { schoolId: params.schoolId, ...summary })
  })
}

export function scheduleParentAttendanceSms(params) {
  if (!params?.schoolId || !params?.studentId) return
  runAfterResponse(async () => {
    const result = await notifyParentAttendance(params)
    console.log('[AttendanceSMS] Done', {
      schoolId: params.schoolId,
