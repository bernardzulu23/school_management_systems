...
import { toSafePupilDto, PUPIL_LIST_USER_SELECT } from '@/lib/privacy/pupilDto'
...
const LIST_INCLUDE = {
  user: { select: PUPIL_LIST_USER_SELECT },
  results: true,
...
function shapeListStudent(s) {
  return toSafePupilDto(
    ...
    { includeFace: false, includeMedical: false, includeGuardianContacts: false }
  )
}
 ...
 * @param {string} userId
 */
export async function findUserById(schoolId, userId) {
  const sid = String(schoolId || '').trim()
  const uid = String(userId || '').trim()
  if (!sid || !uid) return null

  return await prisma.user.findFirst({
    where: { id: uid, schoolId: sid },
    include: {
      studentProfile: true,
      teacherProfile: true,
      hodProfile: true,
...
export async function findStudentsByClass(schoolId, className, page = 1, limit = 20) {
  ...
  return { students: students.map(shapeListStudent), total }
}
...
export async function searchStudents(schoolId, query, { page = 1, limit = 20 } = {}) {
  ...
  const [students, total] = await Promise.all([
    prisma.student.findMany({
      ...
      select: {
        ...
        exam_number: true,
        userId: true,
        user: { select: { id: true, email: true, gender: true } },
      },
      orderBy: { name: 'asc' },
      skip,
      take: limit,
...
/**
 * @param {string} schoolId — required tenant scope (never omit)
 * @param {string} userId
 */
export async function findStudentByUserId(schoolId, userId) {
  const sid = String(schoolId || '').trim()
  const uid = String(userId || '').trim()
  if (!sid || !uid) return null

  const student = await prisma.student.findFirst({
    where: { schoolId: sid, userId: uid },
    include: {
      user: {
        select: {
          id: true,
