...
export function planIncludes(schoolPlan, featureId, school = null) {
  ...
  if (plan === 'individual_premium' || plan === 'individual_annual') {
    ...
    return [...PLAN_FEATURES.individual_free, ...features].includes(featureId)
  }

  if (plan === 'individual_free') {
    return PLAN_FEATURES.individual_free.includes(featureId)
  }

  if (plan === 'student_free') {
    return PLAN_FEATURES.student_free.includes(featureId)
  }
