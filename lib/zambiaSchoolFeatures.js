...
/** Features hidden for primary schools (ECE–Grade 7). Multi-level AI tools are NOT listed here. */
...
/**
 * Pricing-aligned school plan matrix (Blue Peak Pricing cards).
 * - basic: core + SMS; no AI
 * - standard: basic + AI makers / practice (50 AI/month); no report-comments / analytics
 * - premium: standard + report comments, unlimited AI, advanced analytics, etc.
 * - trial: planIncludes allows all (test everything); AI quota is separate (10)
 */
...
/** Lowest paid school plan that includes this feature (for upgrade messaging). */
export function minPlanForFeature(featureId) {
  const id = String(featureId || '').trim()
  if (!id) return 'premium'
  if (PLAN_FEATURES.basic.includes(id)) return 'basic'
  if (PLAN_FEATURES.standard.includes(id)) return 'standard'
  return 'premium'
}

export function planIncludes(schoolPlan, featureId, school = null) {
  const plan = String(schoolPlan || 'basic')
    .trim()
    .toLowerCase()
  const id = String(featureId || '').trim()
  if (!id) return false

  // Trial / active trial window: unlock all features for testing (AI quota is separate).
  ...
  if (plan === 'individual_premium' || plan === 'individual_annual') {
    ...
    return [...PLAN_FEATURES.individual, ...features].includes(id)
  }

  if (plan === 'individual' || plan === 'individual_free') {
    return PLAN_FEATURES.individual.includes(id)
  }

  if (plan === 'standard') {
    return [...PLAN_FEATURES.basic, ...PLAN_FEATURES.standard].includes(id)
  }

  // basic (and unknown → basic)
  return PLAN_FEATURES.basic.includes(id)
}
