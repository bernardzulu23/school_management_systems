...
export async function resolveReviewerUserId({ schoolId, teacherUserId, grade, subject }) {
  ...
  if (isPrimary) {
    const seniorAssignments = await prisma.seniorTeacherAssignment.findMany({
      where: { schoolId, active: true, revokedAt: null },
      select: { userId: true, assignedAt: true },
      orderBy: [{ assignedAt: 'asc' }, { userId: 'asc' }],
      take: 100,
    })
    if (seniorAssignments.length > 0) {
      const index = stableIndex(`${teacherUserId}:${grade}:${subject}`, seniorAssignments.length)
      return seniorAssignments[index]?.userId || null
    }
