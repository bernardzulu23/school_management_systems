...
import { IDLE_TIMEOUT_MS } from '@/lib/security/sessionIdle'
    ...
    (set, get) => ({
      ...
      login: async (credentials) => {
        try {
          ...
          if (error?.response?.status === 403 && error.response?.data?.code === 'WEAK_PASSWORD') {
            throw new Error(
              error.response?.data?.message ||
                'Your password is too weak. Use Forgot Password to set a stronger one.'
            )
          }
          const errorMessage =
      ...
      syncSession: async ({ force = false, strict = false } = {}) => {
        try {
          ...
          const maxIdleMs = IDLE_TIMEOUT_MS
          ...
          sessionSyncInFlight = (async () => {
              ...
              if (meRes.ok) {
                ...
                if (data?.user) {
                  ...
                  set({
                    ...
                    isAuthenticated: true,
                    lastActivityAt:
                      prev.lastActivityAt && prev.lastActivityAt > 0
                        ? prev.lastActivityAt
                        : Date.now(),
                  })
            ...
            const res = await fetch(
              '/api/auth/me',
              withBrowserSessionFetchInit({ cache: 'no-store', credentials: 'include' })
            )
            ...
            if (data?.user) {
              ...
              set({
                ...
                isAuthenticated: true,
                lastActivityAt:
                  prev.lastActivityAt && prev.lastActivityAt > 0 ? prev.lastActivityAt : Date.now(),
              })
              return { success: true, user: data.user }
            }
