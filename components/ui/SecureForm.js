  ...
  const handleSubmit = async (e) => {
    ...
    if (warnings.length > 0) {
      ...
      if (criticalWarnings.length > 0) {
        toast.error('Security issue detected. Please review your input.')
        return
      }
    } else {
      setIsSecure(true)
  ...
  return (
    <div className="relative">
      {showSecurityIndicator && (
        <SecurityIndicator isSecure={isSecure} warnings={securityWarnings} />
      )}

      <form
        onSubmit={handleSubmit}
        onChange={handleInputChange}
        className={`${className} ${!isSecure ? 'border-royalPurple-border bg-royalPurple-danger' : ''}`}
        {...props}
      >
        {enableCSRF && <input type="hidden" name="_token" value={csrfToken} />}

        {children}

        {securityWarnings.length > 0 && <SecurityWarnings warnings={securityWarnings} />}
      </form>
    </div>
  )
}
...
function SecurityIndicator({ isSecure, warnings }) {
  return (
    <div className="flex items-center gap-2 mb-4 p-2 rounded-lg bg-royalPurple-page">
      {isSecure ? (
        <>
          <Shield className="w-4 h-4 text-royalPurple-successTx" />
          <span className="text-sm text-royalPurple-successTx font-medium">Form is secure</span>
        </>
      ) : (
        <>
        ...
        {warnings.map((warning, index) => (
          <li key={index} className="text-sm text-yellow-700">
            ...
            {warning.level === 'critical' && (
              <span className="ml-2 px-2 py-1 bg-royalPurple-danger text-royalPurple-dangerTx text-xs rounded">
                Critical
              </span>
            )}
          </li>
  ...
  return (
    <div className="relative">
      <input
        ...
        className={`
          ${className}
          ${hasWarning ? 'border-royalPurple-border bg-royalPurple-danger focus:border-royalPurple-border focus:ring-red-500' : ''}
        `}
        {...props}
      />
      ...
      {hasWarning && (
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
          <AlertTriangle className="w-4 h-4 text-royalPurple-dangerTx" />
        </div>
      )}
    </div>
  )
