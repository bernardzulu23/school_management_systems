...
export default function HodManagement() {
  ...
  return (
    <main className="space-y-6" aria-labelledby="management-title">
      ...
      <header className="flex items-center justify-between">
        <div>
          <h2 id="management-title" className="text-2xl font-bold text-gray-900 flex items-center">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-6" aria-label="HOD Statistics">
        <Card role="region" aria-labelledby="stat-total-hods">
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p id="stat-total-hods" className="text-sm font-medium text-gray-600">Total HODs</p>
                <p className="text-2xl font-bold text-gray-900">{loading ? <SkeletonLoader className="h-8 w-10" /> : stats.totalHods}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card role="region" aria-labelledby="stat-departments">
          <CardContent className="p-6">
            <div className="flex items-center">
              <Building className="h-8 w-8 text-green-600" aria-hidden="true" />
              <div className="ml-4">
                <p id="stat-departments" className="text-sm font-medium text-gray-600">Departments</p>
                <p className="text-2xl font-bold text-gray-900">{loading ? <SkeletonLoader className="h-8 w-10" /> : stats.departments}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card role="region" aria-labelledby="stat-avg-rating">
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p id="stat-avg-rating" className="text-sm font-medium text-gray-600">Avg Rating</p>
                <p className="text-2xl font-bold text-gray-900">{loading ? <SkeletonLoader className="h-8 w-10" /> : stats.averageRating}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card role="region" aria-labelledby="stat-experience">
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p id="stat-experience" className="text-sm font-medium text-gray-600">Total Experience</p>
                <p className="text-2xl font-bold text-gray-900">
                  {loading ? <SkeletonLoader className="h-8 w-16" /> : `${stats.totalExperience} years`}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <section aria-labelledby="list-title">
        <Card>
          ...
          <CardContent>
            {loading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="status" aria-label="Loading HODs" aria-busy="true">
                {[1, 2, 3].map((n) => (
              ...
              <div className="text-center py-8">
                <UserCheck className="h-12 w-12 text-gray-400 mx-auto mb-4" aria-hidden="true" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" role="list">
                {hods.map((hod) => (
                  <Card key={hod.id} className="hover:shadow-lg transition-shadow focus-within:ring-2 focus-within:ring-blue-500" role="listitem">
                    <CardContent className="p-6">
                      <article className="flex items-start justify-between mb-4">
                        ...
                        <nav className="flex space-x-1" aria-label={`Actions for ${hod.profiles?.name}`}>
                          <Button
                            size="sm"
                            variant="outline"
                          ...
                          <Button
                            ...
                            className="text-red-600 hover:text-red-700 focus-visible:ring-2 focus-visible:ring-red-500"
                          >
                      ...
                      <footer className="mt-4 pt-4 border-t">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-1" aria-label={`Performance rating: ${hod.performance_rating || 0} out of 5 stars`}>
                            {getRatingStars(hod.performance_rating || 0)}
                            <span className={`text-sm font-medium ml-1 ${getRatingColor(hod.performance_rating || 0)}`} aria-hidden="true">
