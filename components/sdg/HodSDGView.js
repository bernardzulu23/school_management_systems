...
export default function HodSDGView({ hodId, departmentData = {} }) {
  ...
  const getDepartmentOverallProgress = () => {
    const totalProgress = Object.values(departmentSDGProgress).reduce(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-royalPurple-pillTx flex items-center justify-center space-x-2">
          <Briefcase className="h-8 w-8" />
          <span>Department SDG Management</span>
        </h1>
        <p className="text-royalPurple-text2 max-w-2xl mx-auto">
          Oversee SDG implementation across your department with budget tracking and staff
          coordination
        </p>
        <div className="flex items-center justify-center space-x-2">
          <span className="text-2xl font-bold text-royalPurple-pillTx">
            {getDepartmentOverallProgress()}%
          </span>
          <span className="text-royalPurple-text2">Department SDG Progress</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="border-l-4 border-l-purple-500">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-royalPurple-pillTx">
              ZMW{' '}
              {budgetAllocation.reduce((sum, item) => sum + item.allocation, 0).toLocaleString()}
            </div>
            <p className="text-xs text-royalPurple-text3">{getBudgetUtilization()}% utilized</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-green-500">
          ...
          <CardContent>
            ...
            <p className="text-xs text-royalPurple-text3">SDG initiatives running</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-orange-500">
          ...
          <CardContent>
            ...
            <p className="text-xs text-royalPurple-text3">Department contribution</p>
          </CardContent>
        </Card>
      </div>
                ...
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-semibold text-royalPurple-text1">
                      SDG {item.sdg}: {item.title}
                    </h3>
                    <p className="text-sm text-royalPurple-text2">{item.programs.join(', ')}</p>
                  </div>
            ...
            {departmentProjects.map((project) => (
              <div key={project.id} className="p-4 border rounded-lg">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-royalPurple-text1">{project.name}</h3>
                  <span
                ...
                <div className="space-y-1 text-sm text-royalPurple-text2">
                  <div>
                    <strong>Budget:</strong> ZMW {project.budget.toLocaleString()}
                  </div>
                  <div>
