...
export default function HODAllocationPage() {
  ...
  const [form, setForm] = useState({
    ...
    periodsPerWeek: 5, blockType: 'DOUBLE',
    singlePeriods: 0, doublePeriods: 0, triplePeriods: 0,
    notes: ''
  })
  ...
  async function loadData() {
    ...
    try {
      ...
      const [teachersData, subjectsData, classesData, allocData] = await Promise.all([
        teachersRes.json(), subjectsRes.json(), classesRes.json(), allocRes.json()
      ])
  ...
  function computeBlockBreakdown(blockType, periodsPerWeek) {
    if (blockType === 'SINGLE') return { singlePeriods: periodsPerWeek, doublePeriods: 0, triplePeriods: 0 }
    if (blockType === 'DOUBLE') return { singlePeriods: 0, doublePeriods: periodsPerWeek / 2, triplePeriods: 0 }
    if (blockType === 'TRIPLE') return { singlePeriods: 0, doublePeriods: 0, triplePeriods: periodsPerWeek / 3 }
    return { singlePeriods: form.singlePeriods, doublePeriods: form.doublePeriods, triplePeriods: form.triplePeriods }
  }
  ...
  async function saveAllocation() {
    ...
    if (form.blockType === 'MIXED' && !validateMixed(form)) {
      ...
      toast.error(`Period breakdown total (${total}) doesn't match periods per week (${form.periodsPerWeek})`)
      return
    }
    ...
    try {
      ...
      setShowForm(false)
      setForm({ teacherId: '', subjectId: '', classId: '', periodsPerWeek: 5, blockType: 'DOUBLE', singlePeriods: 0, doublePeriods: 0, triplePeriods: 0, notes: '' })
      loadData()
    } catch {
  ...
  async function pushToHeadteacher() {
    ...
    if (draftCount === 0) { toast.error('No draft allocations to push'); return }
    ...
    <div style={{ padding: '1.5rem', background: '#170d28', minHeight: '100vh', color: '#ede9fe' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <input value={academicYear} onChange={e => setAcademicYear(e.target.value)} style={{ ...selectStyle, width: 90 }} placeholder="Year" />
          ...
          {draftAllocations.length > 0 && (
            <button onClick={pushToHeadteacher} disabled={pushing} style={btnStyle('#16a34a')}>
              <Send size={14} /> {pushing ? 'Sending...' : `Push ${draftAllocations.length} to Headteacher`}
            </button>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10, marginBottom: '1.5rem' }}>
        {[
          { icon: <Users size={16} />, label: 'Teachers assigned', value: [...new Set(allocations.map(a => a.teacherId))].length },
          ...
          { icon: <CheckCircle size={16} />, label: 'Pushed to HT', value: pushedAllocations.length, green: true },
        ].map((s, i) => (
          <div key={i} style={{ background: '#2d1f4e', borderRadius: 10, padding: '12px 16px', border: '1px solid #3b2a66' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#a78bfa', marginBottom: 4 }}>{s.icon}</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: s.green ? '#86efac' : '#ede9fe' }}>{s.value}</div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#2d1f4e', borderRadius: 16, padding: '1.5rem', width: '100%', maxWidth: 560, border: '1px solid #3b2a66', maxHeight: '90vh', overflowY: 'auto' }}>
            <h2 style={{ fontSize: 17, fontWeight: 700, marginBottom: '1rem' }}>New Class Allocation</h2>

            <FormRow label="Teacher *">
              <select value={form.teacherId} onChange={e => setForm(f => ({ ...f, teacherId: e.target.value }))} style={inputStyle}>
                <option value="">Select teacher</option>
                {teachers.map(t => <option key={t.id} value={t.id}>{t.name} {t.teacherProfile?.department ? `(${t.teacherProfile.department})` : ''}</option>)}
              </select>
            </FormRow>
            ...
            <FormRow label="Periods per week *">
              <input type="number" min={1} max={20} value={form.periodsPerWeek}
            ...
            <FormRow label="Block type *">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                {BLOCK_TYPES.map(bt => (
                  <button key={bt.id} onClick={() => setForm(f => ({ ...f, blockType: bt.id }))}
                    style={{
                      padding: '8px 10px', borderRadius: 8, border: `1px solid ${form.blockType === bt.id ? bt.color : '#3b2a66'}`,
            ...
            {form.blockType === 'MIXED' && (
              <div style={{ background: '#261843', borderRadius: 8, padding: '12px', marginBottom: 12, border: '1px solid #3b2a66' }}>
                <p style={{ fontSize: 12, color: '#a78bfa', marginBottom: 8 }}>Breakdown (must total {form.periodsPerWeek} periods):</p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                  {[['Triple periods (×3)', 'triplePeriods', '#f59e0b'], ['Double periods (×2)', 'doublePeriods', '#8b5cf6'], ['Single periods (×1)', 'singlePeriods', '#3b82f6']].map(([label, key, color]) => (
                    <div key={key}>
                      <label style={{ fontSize: 10, color, display: 'block', marginBottom: 4 }}>{label}</label>
                ...
                {(() => {
                  ...
                  return <p style={{ fontSize: 11, color: ok ? '#86efac' : '#fca5a5', marginTop: 6 }}>
                    {ok ? `✓ ${total} periods = ${form.periodsPerWeek}` : `✗ ${total} ≠ ${form.periodsPerWeek} (adjust breakdown)`}
                  </p>
                })()}
              </div>
            )}
            ...
            <FormRow label="Notes">
              <input value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder="Optional notes for the headteacher" style={inputStyle} />
            </FormRow>

            {/* Preview */}
            {form.teacherId && form.subjectId && form.classId && (
              <div style={{ background: '#170d28', borderRadius: 8, padding: '10px 12px', marginBottom: 12, border: '1px solid #7c3aed' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
              <button onClick={() => setShowForm(false)} style={btnStyle('#261843')}>Cancel</button>
              <button onClick={saveAllocation} style={btnStyle('#7c3aed')}>Save Allocation</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Allocations table */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#6d28d9' }}>Loading allocations...</div>
      ) : allocations.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem', background: '#2d1f4e', borderRadius: 14, border: '1px solid #3b2a66' }}>
          <p style={{ color: '#6d28d9', marginBottom: 12 }}>No allocations yet for {term} {academicYear}</p>
          <button onClick={() => setShowForm(true)} style={btnStyle('#7c3aed')}><Plus size={14} /> Add First Allocation</button>
        </div>
      ) : (
        <div style={{ background: '#2d1f4e', borderRadius: 14, border: '1px solid #3b2a66', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#170d28' }}>
                {['Teacher', 'Subject', 'Class', 'Periods / Block', 'Total min/week', 'Status', ''].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, color: '#6d28d9', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid #3b2a66' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {allocations.map(a => (
                <tr key={a.id} style={{ borderBottom: '1px solid #261843' }}>
                  ...
                  <td style={tdStyle}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#ede9fe' }}>{a.periodsPerWeek} periods</div>
...
function FormRow({ label, children }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <label style={{ display: 'block', fontSize: 12, color: '#a78bfa', fontWeight: 600, marginBottom: 5 }}>{label}</label>
