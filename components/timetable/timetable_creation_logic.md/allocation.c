...
export async function GET(req) {
  ...
  const allocations = await prisma.teacherAllocation.findMany({
    ...
    include: {
      ...
      class: { select: { id: true, name: true, year: true, stream: true } },
    },
...
export async function POST(req) {
  ...
  let finalSingle = singlePeriods, finalDouble = doublePeriods, finalTriple = triplePeriods
  if (blockType === 'SINGLE') {
    finalSingle = periodsPerWeek; finalDouble = 0; finalTriple = 0
  } else if (blockType === 'DOUBLE') {
    // must be even number
    if (periodsPerWeek % 2 !== 0) {
      return NextResponse.json({ error: 'DOUBLE blockType requires an even number of periods' }, { status: 400 })
    }
    finalDouble = periodsPerWeek / 2; finalSingle = 0; finalTriple = 0
  } else if (blockType === 'TRIPLE') {
    if (periodsPerWeek % 3 !== 0) {
      return NextResponse.json({ error: 'TRIPLE blockType requires periods divisible by 3' }, { status: 400 })
    }
    finalTriple = periodsPerWeek / 3; finalSingle = 0; finalDouble = 0
  }
    ...
    const allocation = await prisma.teacherAllocation.upsert({
      where: {
        schoolId_teacherId_subjectId_classId_term_academicYear: {
          schoolId, teacherId, subjectId, classId, term, academicYear
        }
      },
      update: {
        periodsPerWeek, blockType,
        singlePeriods: finalSingle, doublePeriods: finalDouble, triplePeriods: finalTriple,
        notes, status: 'draft', hodId: user.id
      },
      create: {
        schoolId, hodId: user.id, teacherId, subjectId, classId,
        periodsPerWeek, blockType,
        singlePeriods: finalSingle, doublePeriods: finalDouble, triplePeriods: finalTriple,
        term, academicYear, notes, status: 'draft'
      },
      include: {
