...
export function CompactWeekTimetableGrid({
  ...
  const legendTeachers = useMemo(() => {
    ...
    for (const a of assignments || []) {
      ...
      map.set(id, {
        id,
        fullName: String((a as any).teacherName || teacherDisplayName(id, 'full') || 'Teacher'),
        colorHex: getTeacherColorHex(id) || teacherColors[id]?.colorHex,
      })
    }
    return Array.from(map.values()).sort((a, b) =>
      String(a.fullName).localeCompare(String(b.fullName), undefined, { sensitivity: 'base' })
    )
  }, [assignments, getTeacherColorHex, teacherColors])
  ...
  return (
    <div className="space-y-3">
      ...
      {showLegend && legendTeachers.length > 0 ? (
        <TeacherColorLegend teachers={legendTeachers} colorMap={teacherColors} />
      ) : null}
    </div>
  )
}
