...
export default function GenerateReportsModal({ students, onClose }) {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedReportType} report generated for ${selectedStudents.length} student(s) in ${reportFormat.toUpperCase()} format!`
      )
      onClose()
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="bg-royalPurple-card border border-royalPurple-border rounded-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              ...
              {reportTypes.map((type) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <type.icon
                      className={`h-6 w-6 ${
                        selectedReportType === type.id ? 'text-current' : 'text-royalPurple-text2'
                      }`}
                      aria-hidden="true"
                    />
                    <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <section aria-labelledby="student-selection-title">
              <div className="flex items-center justify-between mb-4">
                <h3 id="student-selection-title" className="font-semibold">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <label htmlFor={`student-${student.id}`} className="flex-1 cursor-pointer">
                        ...
                        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
                          <div className="flex items-center">
                            <TrendingDown
                              className="h-3 w-3 text-royalPurple-dangerTx mr-1"
                              aria-hidden="true"
                            />
                            <span>
                              {student.subjects.filter((s) => s.score < 40).length} failing
                            </span>
                          </div>
                          <div className="flex items-center">
                            <Users
            ...
            <div className="space-y-6">
              ...
              <section aria-labelledby="format-title">
                <h3 id="format-title" className="font-semibold mb-3">
                  Report Format:
                </h3>
              ...
              <section aria-labelledby="options-title">
                <h3 id="options-title" className="font-semibold mb-3">
                  Report Options:
                </h3>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-royalPurple-text2">
                        Add intervention strategies and action plans
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-royalPurple-text2">
                        Add parent/guardian contact details
                      </p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
              ...
              {/* Report Preview */}
              <section
                aria-label="Report summary preview"
                className="p-4 bg-royalPurple-page border rounded-lg"
              >
                <h4 className="font-medium mb-2">Report Preview:</h4>
                <div className="text-sm text-royalPurple-text2 space-y-1">
                  <div>
                    • Report Type: {reportTypes.find((t) => t.id === selectedReportType)?.name}
                  </div>
                  <div>• Format: {formats.find((f) => f.id === reportFormat)?.name}</div>
                  <div>• Students: {selectedStudents.length} selected</div>
                  <div>
                    • Recommendations: {includeRecommendations ? 'Included' : 'Not included'}
                  </div>
        ...
        {/* Action Buttons */}
        <footer className="flex space-x-4 p-6 border-t sticky bottom-0 bg-royalPurple-card">
          <Button
            ...
            aria-label={
              isGenerating
                ? 'Generating report...'
                : `Generate report for ${selectedStudents.length} students`
            }
          >
            {isGenerating ? (
