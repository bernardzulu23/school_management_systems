...
- **Fixes:**
  1.  Implemented `lru-cache` based rate limiter in the Next.js proxy layer.
  2.  Configured strict Content Security Policy (CSP), HSTS, and other security headers in `proxy.js`.
...
- **Fixes:**
  ...
  2.  Integrated auth and RBAC checks into the global Next.js proxy.
...
- **Fix**: Updated `rateLimiter.js` and `proxy.js` to use a 15-minute window and 5-request limit for auth routes.
