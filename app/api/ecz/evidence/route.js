...
import { isEczStaff, isEczManager, EVIDENCE_STATUS_FILTERS } from '@/lib/ecz/routeAuth'
...
export const GET = withSecureHandler(async function GET(request) {
  ...
  const scoreId = safeStringId(searchParams.get('scoreId')) || undefined
...
export const POST = withSecureHandler(async function POST(request) {
  ...
  const scoreId = safeStringId(formData.get('scoreId'))
