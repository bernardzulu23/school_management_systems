...
import { ArrowLeft, Download, RefreshCw, BookOpen, ClipboardList, PenLine } from 'lucide-react'
    ...
    <DashboardLayout>
      <div className="space-y-6 p-4 md:p-6 max-w-6xl mx-auto">
        ...
        <div className="rounded-lg border border-blue-200 bg-blue-50/80 p-4 text-sm text-blue-900">
          <strong>ECZ compliance:</strong> Forms 1–3 use SBA (30%) + final exam (70%). Form 4 has no
          SBA. Every SBA task needs Zambian context. Submit scores by <strong>31 January</strong>.
          Use the 4-level rubric: Excellent (4), Good (3), Fair (2), Needs Improvement (1).
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">19 from guidelines</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
          ...
          <CardContent>
            {loading ? (
              ...
              <p className="text-sm text-muted-foreground">
                No ECZ assessments yet. Sync subjects (19 from ECZ guidelines), then create an SBA
                task with Zambian context.
              </p>
            ) : (
                ...
                {tasks.map((t) => (
                  <li key={t.id} className="py-3 flex justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {t.subject?.name} · Form {t.formLevel} · {t.component?.replace(/_/g, ' ')} ·{' '}
                        {t.type}
