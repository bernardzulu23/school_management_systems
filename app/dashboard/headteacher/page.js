  ...
  const handleRegistration = async (formData, userType) => {
    ...
    try {
      ...
      if (response.status >= 400) {
        ...
        if (code === 'DUPLICATE_EXAM_NUMBER') {
          throw new Error(
            'This exam number is already registered. Please use a unique ECZ exam number.'
          )
        } else if (code === 'PAYMENT_REQUIRED') {
          throw new Error(
            'Student registration requires an active school plan. Please upgrade your plan.'
          )
        } else if (code === 'DUPLICATE_USER') {
          throw new Error('A user with this email already exists in your school.')
        }
      ...
      if (response.data.success) {
        ...
        const password = response.data.autoGeneratedPassword
        if (password) {
          toast.success(
            <div>
