...
export default function SeniorTeacherLessonPlansPage() {
  ...
  const pendingCount = useMemo(
    () => plans.filter((p) => String(p?.status || '').toUpperCase() === 'SUBMITTED').length,
    [plans]
  )

  return (
    <DashboardLayout title="Lesson Plans (Senior Teacher)">
      <div className="space-y-4">
        <Link href="/dashboard/senior-teacher">
                    ...
                    plans.map((p) => (
                      <tr key={p.id} className="border-b border-royalPurple-border/40">
                        ...
                        <td className="py-3 text-right">
                          <Link href={`/dashboard/senior-teacher/lesson-plans/${p.id}`}>
