...
export default function SubjectsPage() {
  ...
  if (loading) {
    return (
      <main className="container mx-auto px-4 py-6" role="main" aria-busy="true" aria-label="Loading subjects">
        <div className="mb-6">
  ...
  return (
    <main className="container mx-auto px-4 py-6" role="main">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <select
                ...
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
              >
      ...
      {/* Statistics */}
      <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6" aria-label="Subject statistics">
        <Card className="p-4 text-center">
          <div className="text-2xl font-bold text-blue-600" role="status">{subjects.length}</div>
          <div className="text-sm text-gray-600">Total Subjects</div>
        </Card>
        <Card className="p-4 text-center">
          <div className="text-2xl font-bold text-green-600" role="status">{categories.length}</div>
          <div className="text-sm text-gray-600">Categories</div>
        </Card>
        <Card className="p-4 text-center">
          <div className="text-2xl font-bold text-purple-600" role="status">8</div>
          <div className="text-sm text-gray-600">Local Languages</div>
        </Card>
        <Card className="p-4 text-center">
          <div className="text-2xl font-bold text-orange-600" role="status">14</div>
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" role="list">
                  {categorySubjects.map((subject) => (
                    <li key={subject.id} role="listitem">
                      <article className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow focus-within:ring-2 focus-within:ring-blue-500">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="font-semibold text-gray-900">{subject.name}</h3>
                          <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded" aria-label={`Subject code: ${subject.code}`}>
                            {subject.code}
                          </span>
                        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200" role="grid">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredSubjects.map((subject) => (
                    <tr key={subject.id} className="hover:bg-gray-50 focus-within:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <span className="bg-gray-100 px-2 py-1 rounded text-xs font-mono" aria-label={`Code: ${subject.code}`}>
                          {subject.code}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
