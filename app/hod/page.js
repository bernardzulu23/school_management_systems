...
export default function HODDashboard() {
  ...
  const renderDutySection = (title, duties, sectionColor) => (
    <div className="bg-royalPurple-card rounded-lg shadow-md p-6 mb-6">
      <h3 className="text-xl font-semibold mb-4 text-royalPurple-text1">{title}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {duties.map((duty, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-royalPurple-page rounded-lg p-4 cursor-pointer hover:shadow-md transition-all duration-300 hover:-translate-y-1"
              onClick={() => handleNavigate(duty.route)}
            >
              <div className="flex items-center mb-2">
                <IconComponent className={`w-6 h-6 mr-3 text-${duty.color}-600`} />
                <h4 className="font-semibold text-royalPurple-text1">{duty.title}</h4>
              </div>
              <p className="text-sm text-royalPurple-text2">{duty.description}</p>
            </div>
          )
        })}
      </div>
  ...
  return (
    <div className="min-h-screen bg-royalPurple-page p-6">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-royalPurple-text1 mb-2">HOD Dashboard</h1>
        <p className="text-royalPurple-text2">Welcome back, Head of Department</p>
      </div>

      {/* Statistics Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-royalPurple-card rounded-lg shadow-md p-6">
          <p className="text-royalPurple-text2 text-sm mb-2">Total Teachers</p>
          <p className="text-3xl font-bold text-royalPurple-accentTx">{stats.totalTeachers}</p>
        </div>
        <div className="bg-royalPurple-card rounded-lg shadow-md p-6">
          <p className="text-royalPurple-text2 text-sm mb-2">Total Students</p>
          <p className="text-3xl font-bold text-royalPurple-successTx">{stats.totalStudents}</p>
        </div>
        <div className="bg-royalPurple-card rounded-lg shadow-md p-6">
          <p className="text-royalPurple-text2 text-sm mb-2">Total Classes</p>
          <p className="text-3xl font-bold text-royalPurple-pillTx">{stats.totalClasses}</p>
        </div>
        <div className="bg-royalPurple-card rounded-lg shadow-md p-6">
          <p className="text-royalPurple-text2 text-sm mb-2">Pending Tasks</p>
          <p className="text-3xl font-bold text-orange-600">{stats.pendingTasks}</p>
        </div>
      </div>

      {/* HOD Administrative Duties */}
      <h2 className="text-2xl font-bold text-royalPurple-text1 mb-6">HOD Administrative Duties</h2>

      {/* File Management Section */}
      {renderDutySection('📁 File Management', hodDuties.fileManagement, 'blue')}

      {/* Academic Management Section */}
      {renderDutySection('🎓 Academic Management', hodDuties.academicManagement, 'green')}

      {/* Daily Operations Section */}
      {renderDutySection('⏰ Daily Operations', hodDuties.dailyOperations, 'purple')}

      {/* Financial Management Section */}
      {renderDutySection('💰 Financial Management', hodDuties.financialManagement, 'orange')}
    </div>
  )
}
